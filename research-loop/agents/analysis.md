...
The opening message is your analysis order in full; take it with `rl handoff start`. Compute only what an approved evaluation names and draw only what gyb asked for; when a grouping key is missing, open the issue to gyb and mark the order stuck; when deploy's code is wrong, open the issue to deploy instead of working around it (whether that order is marked stuck too: PENDING(part 24 L88)). Finish with the order id, the notebook and figure paths, and any issue that waits on someone else.
