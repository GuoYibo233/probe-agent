...
<!-- Sources: 08 L90 and 06 L126 (shape only, no hooks), 12 L11 and tables/roles/run.json (model opus after gyb's 2026-08-16 re-ruling), 11 L89 and 12 L417 (the opening message copies the launch order), principle-11 (background dispatch; the GPU job itself lives in tmux). Tool face derived from the use-case table in skills/run/SKILL.md: Bash for rl, the host launcher, tmux and the watchdog, Read/Grep/Glob for the GPU state file, code and logs, Write/Edit for files under artifact_root, Skill for the host's launch methodology reference; no Agent, run dispatches to nobody. The spelling of the preloaded skill name is unverified: PENDING(D-13). -->
