...
## Header
The frontmatter carries `name` and `description`. It declares no hooks: hooks are plugin-level and bind to role directories, never to a skill.
...
## Section 2: Use cases
A table with four columns: use case; ledgers and directories read; directory written; `rl` write commands. Each row is one use case, and the role json's four columns are derived from these rows, never the other way round (principle-05). Below the table, one short paragraph per use case in working order: what triggers it, what to read first, what to write, where it ends.
