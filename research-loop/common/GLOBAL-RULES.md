...
Role skills point here with one line, "Follow `common/`", and never copy these entries (09 L27). Numbers `rule-NN` and `principle-NN` never change once assigned: a retired entry leaves its number empty, a new entry takes the next number, a changed entry keeps its number and only its text changes. Feedback targets and decision sources cite these numbers, so an old citation still resolves.

## Rules

### rule-01 Default at an unwritten fork
When the spec does not cover a fork, the role keeps working by the eleven principles below and appends one decision whose source states the basis. Writes made in gyb's voice (`--as-gyb`, approving an evaluation, ruling on feedback) stop hard unless gyb said it in person just now. Irreversible actions (withdrawing, retiring) stop hard and carry gyb's words or a reason.

Where the two irreversible actions land (09 L51): withdrawing an order is `rl handoff withdraw --reason`, and a withdrawal issued from a role session also carries `--quote` (04 L40); retiring a decision is `rl decision retire`, which always carries a reason, whoever retires it and from whichever terminal, and the reason is recorded on that version of the decision row. gyb's words for a retirement done on gyb's behalf inside a role session travel by the existing `--as-gyb --quote` identity rule; nothing is added for it.
