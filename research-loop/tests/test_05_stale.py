...
Changed by 05-rl-cli.md (30 L68, 2026-08-17 裁): `rl inbox` item 3 lists only the orders
held by the CURRENT SESSION, not everything the role owns - an owned-but-not-held stale
order goes to `rl status` section 6 instead. `rl decision stale` dropped `--mine`; its
signature is `[--handoff ID] [--all]`, and with neither flag it defaults to the current
session's held orders (02 L89), the same "current session" rule `rl inbox` item 3 uses.
"""
...
class StaleDecisions(unittest.TestCase):
    ...
    def test_order_citing_v1_is_listed_stale_with_all_after_update_to_v2(self):
        # 30 L66: "派活单引 v1、决定更新到 v2 之后 `rl decision stale` 列出来".
    ...
    def test_handoff_filter_lists_only_that_orders_stale_refs(self):
        # 02 L89: "--handoff ID 只查那张单子引的"; 05 signature `[--handoff ID] [--all]`.
    ...
    def test_default_lists_only_orders_held_by_the_current_session(self):
        # 30 L68 (2026-08-17 裁): default = "只列和本会话手上单子有关的".
        dec = make_decision(self.sb)
        ho = open_work_order(self.sb, None, dec)
    ...
    def test_confirm_does_not_make_a_citing_order_stale(self):
        # 02 L60, L85: confirm "不算改版，不触发过版"; 30 L66 backdrop (also asserted,
        # from the decision-account side, in test_03_decisions.py).
        dec = make_decision(self.sb)
        ho = open_work_order(self.sb, None, dec)
    ...
    def test_evaluation_references_checked_for_staleness_is_undecided(self):
        # 30 L68: "口径引用同样查过版" carries over the original sentence, but no command
        # is named for it; 22-pair-idea-analysis.md L115 lists this as one of the
        # questions left for gyb (item 3 of "源文档没写清的"), still unruled.
        self.skipTest(
...
class Step4Commands(unittest.TestCase):
    """`rl status` and `rl inbox` are step-4 deliverables (30 L197 step table, row '4 交流
    机制'); step 3 has no scripts/rl_cmds handler for them, so these stay red - by design -
    until step 4 lands. 30 L66 and 30 L68 name both commands as part of test 5's original
    scope."""
    ...
    def test_status_section_6_lists_the_stale_order(self):
        # 05-rl-cli.md L160: section 6 = "过版的单子和 retired 决定名下的活单".
        dec = make_decision(self.sb)
        ho = open_work_order(self.sb, None, dec)
        self.sb.rl("decision", "update", dec, "--text", "revised approach")
    ...
    def test_inbox_item_3_lists_only_orders_held_by_this_session(self):
        # 05-rl-cli.md L137, L143 (2026-08-17 裁): item 3 = "本会话手上单子引的过版决定".
        dec = make_decision(self.sb)
        ho = open_work_order(self.sb, None, dec)
