"""T7②抽取头训练:ModernBERT 底座 + 起止指针头 + 可答头(C2-2t)。

- 输入: bert_data/v3/<env>/{train,calA}.jsonl(文本) 与
  bert_data/v3_params/<env>/同名文件(参数区间标签),按 event+sent_idx 连接
...
- 判对口径两套(BPE 会把前导空格/引号并进 token,严格逐字会低估):
    ...
    严格(对照)   预测区间解码文本逐字 == 真值
- 每轮 calA 报: 可答准确率 / span 命中(宽松·严格) / 参数级总正确率,按 w 加权
- 产物: <out>/best/(model.pt + tokenizer + meta.json)+ train_log.jsonl

用法(smoke): python train_extractor.py --env bfcl --out <dir> --smoke
"""
...
# ---------- 数据:v3 文本 × v3_params 区间 ----------

def join_rows(data, params, split, limit=0):
    """流式并归(两文件同序,params 是 v3 的子序列)-> (texts, instances)。"""
    texts, inst = [], []
...
def load_extractor(run, device="cuda"):
    """eval_extract 复用:返回 (model, tok, meta)。"""
    run = Path(run)
...
def main():
    ...
    params = Path(args.params) / args.env
    out = Path(args.out)
    ...
    ev = InstDS(*join_rows(data, params, "calA", lim_ev))
