"""T7③抽取头触发时刻评测:回放定出触发点,在该前缀上抽参数,报完整调用正确率。

- 触发点:test.jsonl 按分类头 label_map 过滤(与 eval_replay 逐行同序),
  probs=softmax(logits_test/T),按回放协议取每事件首次过 θ 的 sent_idx
- 在触发样本的 text 上对该事件每个参数跑抽取头;真值 found/区间取的正是
  **该触发前缀**上的标签,所以"值还没出现"天然记为抽不到
- 参数正确 = (真值 found 且预测区间对) or (真值抽不到且预测抽不到);
  区间对分宽松(主口径,允许多出空白/标点)与严格(逐字)两套
- 事件参数全对 = 该事件全部参数正确(无参事件恒真,单独成列)
- 完整调用正确 = 分类预测对 × 参数全对
- 分三档(param_tiers 读 <data>/<env>/router_stats.md)报数,产 EXTRACT_REPORT.{json,md}

用法: python eval_extract.py --env bfcl --run <分类头run> --extractor <抽取头run>
"""
...
from param_tiers import parse, tier_of
from train_extractor import FIND, collate, decode, load_extractor, span_ok
...
def replay_fire(rows, probs, theta):
    """与 eval_replay.replay 同逻辑,额外返回触发的 sent_idx 与行。"""
    ev = defaultdict(list)
...
def main():
    ...
    ap.add_argument("--run", required=True, help="分类头 run 目录")
    ap.add_argument("--data", default=str(BASE / "bert_data" / "v3"))
    ap.add_argument("--params", default=str(BASE / "bert_data" / "v3_params"))
    ap.add_argument("--extractor", required=True, help="抽取头 run 目录")
    ...
    # 1) 触发点:过滤逻辑与 eval_replay 逐行一致,保证与 logits_test.pt 同序
    ...
    # 2) 参数真值:v3_params 里同一 (event, sent_idx) 那一行
    pmap = {}
    for p in load_rows(Path(args.params) / args.env / "test.jsonl"):
