"""因果探针训练:因果语言模型底座 + 线性分类头(C2-2t T8)。

与 ModernBERT 探针(train_probe.py)的本质区别:ModernBERT 每个句子边界重编码
...
- 输入: bert_data/v3/<env>/{train,calA}.jsonl + tool_vocab.json
- 底座: --base qwen -> Qwen3-0.6B-Base / --base lfm -> LFM2.5-350M-Base(AutoModel)
- 截断: fast tokenizer 左截 --max-len(保思考尾巴),offset_mapping 指向原字符串;
  左截丢掉的边界跳过并计数(n_bound_dropped)
- 评估: 每轮 calA 报 calA_weighted_acc + calA_lastbound_acc(与 train_probe 同名)
...
用法:
  # 只过对齐检查
  python train_causal_probe.py --base qwen --env bfcl --out <dir> --align-only
  # 冒烟
  python train_causal_probe.py --base qwen --env bfcl --out <dir> --smoke
  # 全量
  python train_causal_probe.py --base qwen --env bfcl --out <dir>
"""
...
MODELS = {
    ...
    "lfm": "/net/tokyo100-10g/data/str01_01/y-guo/models/LFM2.5-350M-Base",
}
SEED = 20260729
ALIGN_TOL = 1e-4
SPOT = 50          # 前缀性质抽查的事件数
...
def main():
    ...
    ap.add_argument("--base", required=True, choices=sorted(MODELS),
                    help="qwen=Qwen3-0.6B-Base / lfm=LFM2.5-350M-Base")
    ...
    ev_events = load_events(data / "calA.jsonl", label2id)
