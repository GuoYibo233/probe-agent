"""Summarize three metrics for the full batch: accuracy / median per-step thinking length
(characters) / calls per trajectory.

Same convention as CALIB_v0.md, for direct comparison with the 5-task calibration batch.
Usage: python summarize_full.py
"""

import json
import statistics
from pathlib import Path

RUNS = Path("/home/y-guo/reproduce/new1/envs/runs")
MODELS = ["q35", "q36", "gptoss"]


def read_traj(path):
    """Returns (meta, gens, envs, final); bad lines are skipped."""
    meta, gens, envs, final = {}, [], [], {}
    for line in path.read_text().splitlines():
        if not line.strip():
            continue
        try:
            r = json.loads(line)
        except json.JSONDecodeError:
            continue
        t = r.get("type")
        if t == "meta":
            meta = r
        elif t == "gen":
            gens.append(r)
        elif t == "env":
            envs.append(r)
        elif t == "final":
            final = r
    return meta, gens, envs, final


def summarize(d, success_fn):
    rows = []
    for p in sorted(d.glob("*.jsonl")):
        meta, gens, envs, final = read_traj(p)
        if not final:
            continue
        think = [len(g.get("reasoning") or "") for g in gens]
        rows.append({
            "ok": success_fn(final),
            "think": think,
            # A step with an action = one real tool call
            "calls": sum(1 for e in envs if e.get("action")),
            "steps": final.get("steps", len(gens)),
        })
    if not rows:
        return None
    all_think = [t for r in rows for t in r["think"]]
    return {
        "n": len(rows),
        "acc": sum(r["ok"] for r in rows) / len(rows),
        "think_med": statistics.median(all_think) if all_think else 0,
        "calls": statistics.mean(r["calls"] for r in rows),
        "steps": statistics.mean(r["steps"] for r in rows),
    }


def aw_ok(final):
    return "'success': True" in str(final.get("eval", ""))


def tl_ok(final):
    return bool(final.get("won"))


def main():
    print(f"{'cell':<28} {'n':>4} {'acc':>6} {'thinking median/step':>11} "
          f"{'calls/traj':>9} {'mean steps':>8}")
    print("-" * 74)
    for env_name, sub, ok_fn in [("AppWorld dev", "full_appworld", aw_ok),
                                 ("TALES L1 6-room", "full_tales_L1", tl_ok),
                                 ("TALES L2 11-room", "full_tales_L2", tl_ok)]:
        for m in MODELS:
            d = RUNS / sub / m
            if not d.exists():
                continue
            s = summarize(d, ok_fn)
            if not s:
                continue
            print(f"{env_name + ' x ' + m:<28} {s['n']:>4} {s['acc']:>6.2f} "
                  f"{s['think_med']:>11.0f} {s['calls']:>9.1f} "
                  f"{s['steps']:>8.1f}")


if __name__ == "__main__":
    main()
