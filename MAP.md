...
| 训练器 debugger 演示 | `demo-prep` 造假件（合成小数据 + 两层小 Qwen3），然后 `.vscode/launch.json` 进 debugger，或 `demo-train --mode cgen --out demo/runs/cgen` 整程跑一遍（纯 CPU，十几秒；说明在 `demo/README.md`） | 无 |
...
| （演示） | `demo/prepare.py` | 给 debugger 演示造假件：合成 AppWorld 风格小数据 `demo/data/{train,val}.jsonl`（切点、题干、调用串分别调标注段的 `rules.boundaries` / `rules.assemble` / `build.make_call`，字段与真数据逐个相同）+ 两层随机初始化 Qwen3 加真分词器 `demo/tiny_qwen3/`；跑完自己用 `build(path=)` / `share_data.load_events` / `chunk_by_budget` 把假件装一遍，印每个事件的长度与装块数 | `python3 run.py demo-prep`（纯 CPU，十秒，只在这一步碰一次 NFS 拷分词器）；进 debugger 用 `.vscode/launch.json` 的两个训练配置（被调试的是真训练器 `train_causal_share.py`，不改一字，靠 `--base demo/tiny_qwen3 --device cpu` 两扇现成的门）；不进 debugger 用 `demo-train --mode cgen --out demo/runs/cgen`；产物 `demo/runs/` 不进库不进矩阵；在哪下断点看 `demo/README.md` |
