...
| `pipeline/collect/gen_launch.py` | 采集批次发射清单生成器：吃 manifest json，吐 `launch_servers.py`/`launch_clients.sh`/`MANIFEST.md`。只生成不执行，真发射走 gpu-run。manifest 顶层可选 `gptoss_client_preset` 选 gpt-oss 客户端预设（缺省 gptoss_chat_high，2026-08-21 起） | `python3 run.py gen-launch --config pipeline/collect/manifest_w0.json`；试生成 `python3 run.py gen-launch --config … --dry-run --out-override /tmp/x/` |
| `pipeline/collect/manifest_*.json` | 采集批次清单（服务器摆位 + 客户端分片）：p1 是现役批（harmony_high 口径），w0/c2 留作模板 | 被 gen_launch.py 读 |
