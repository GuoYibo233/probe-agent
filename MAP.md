...
| `pipeline/collect/gen_launch.py` | 采集批次发射清单生成器：吃 manifest json，吐 `launch_servers.py`/`launch_clients.sh`/`MANIFEST.md`。只生成不执行，真发射走 gpu-run。manifest 顶层可选 `gptoss_client_preset` 选 gpt-oss 客户端预设（缺省 gptoss_chat_high，2026-08-21 起）；可选 `traj_per_task`+`seed_family`（要么都给要么都不给，长度必须相等，只认 env=appworld，2026-08-22 起）给客户端命令追加 `--traj-per-task N --seeds …`，不给时生成物与旧版逐字节一致 | `python3 run.py gen-launch --config pipeline/collect/manifest_w0.json`；试生成 `python3 run.py gen-launch --config … --dry-run --out-override /tmp/x/` |
| `pipeline/collect/manifest_*.json` | 采集批次清单（服务器摆位 + 客户端分片）：np821 是现役批（default 预设、温度 1、每题 4 条），p1 是上一批（harmony_high 口径），w0/c2 留作模板 | 被 gen_launch.py 读 |
...
| `configs/models.json` + `configs/presets/*.json` | 生成设置的唯一真源：models.json 是模型地址映射；一份预设 = 模型别名 + server 节（vLLM 启动参数）+ client 节（api/effort/temperature/max_tokens/stop/start_date/top_p/seed）。现有八份预设：五份 gptoss_* 与改造前五处写死值逐项等价（tests/test_preset.py 钉着），`gptoss_harmony_high` 是 p1 采集口径（effort high，2026-08-21 加），`gptoss_default` 是 OpenAI 官方推荐口径（temperature 1.0 / top_p 1.0 / top_k 0 / min_p 0 / effort medium / 上下文 131072，2026-08-20 加），`default` 是 np821 采集口径（harmony / effort high / temperature 1.0 / top_p 1.0 / max_tokens 8192 / 无 server 节，种子由采集器 `--seeds` 按轨迹派，2026-08-22 加） | 采集/活跑/回放入口 `--preset <名>`；BFCL handler 走环境变量 `NEW1_PRESET_JSON=<预设绝对路径>`；命令行显式参数永远压过预设 |
