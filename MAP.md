...
| 程序 | 干什么 | 怎么用 |
|---|---|---|
| `pipeline/inject/live_appworld.py` | 驱动器：整题活跑（with probe / no probe / probe-but-nofill 三臂）。prompt 以 token id 发（/render 前缀），流带 `return_token_ids`，不出手的步与 chat 端点喂引擎的 id 串相同；开火重发 = 前缀 + **模型自己生成的 id[:k]**（切口退到 token 边界，/decode 核对）+ /encode(NOTE)，不再整段重分词（v6 2026-08-18 ident3）。每个新句子级切口问探针（或 `--fire-nth-cut N` 伪触发），首过 θ 触发注入（默认每步至多 1 次；`--nofill` 只中断重发不塞），`<\|end\|>` 后收尾；每题落一个 `live_<task_id>.jsonl`（meta/gen(含 gen_ids)/spec(含 overflow_ids)/resume/env/final）。自带 `--selftest-shadow <轨迹>`（无服务验证三连不污染正身） | `python3 run.py live-appworld --base-url <vLLM /v1> --probe-url <probe_server> --split test_normal --outdir … --exp …`（发射类）；⚠️ 只能 `envs/appworld/venv/bin/python` 跑；对照臂加 `--no-probe`；probe-but-nofill 臂加 `--fire-nth-cut 5 --nofill`；分片 `--num-shards/--shard-id`；默认跑完即删 appworld 输出目录 |
