...
| ctool | `pipeline/train/train_causal_tool.py` | Qwen3 底座 + 线性分类头：整段一次前向、每个句边界放监督 | `--base` 必填，三档 qwen(0.6B)/qwen17(1.7B)/qwen4(4B)；**开训前对齐检查是铁律**（整段 vs 逐 token 增量，maxdiff<tol，FAIL 退 2；参考值 `--align-tol 3e-4`）；48G 卡常 OOM → `--grad-ckpt`；`--lora` 只把底座换成 LoRA 训（分类头照常全参，见 lora_util 行） |
| cgen | `pipeline/train/train_causal_callgen.py` | Qwen3 底座微调：直接续写整条调用（`\n[CALL] ` 分隔） | `--base` 默认 qwen，qwen17/qwen4 换档（2026-08-21 起）；选 best 只看 val 加权 CE；48G 卡需 `--grad-ckpt`（大数据侧仍可能 OOM，旧阶段迁 H200 才跑完过）；`--lora` 把底座换成 LoRA 训（开火头照常全参，见 lora_util 行） |
| cparam | `pipeline/train/train_causal_param.py` | Qwen3 底座微调：给定工具名只写参数段——输入到 `…[CALL] 工具名(` 为止，目标是括号里的参数加右括号（从 `label_call` 按前缀剥离，对不上整条丢弃计 `assembly_mismatch`） | `--base` 默认 qwen；数据与 cgen 同源零新标注；选 best 看 val 加权 CE（`val_exact_params` 只进日志）；没有 `--fire-head`；meta 带 `param_only: true`；`--lora` 把底座换成 LoRA 训（见 lora_util 行） |
