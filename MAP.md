...
| `ops/gpu_jobs.py` | GPU 台账 CLI：register / finish / watch / free / status / json。free/register/finish 卡的实时占用永远现场探测；status/watch/json 三个终端出口改读采样器（`ops/sampler.py`）落盘的 `latest.json`（工单 07）：最后采样时刻在 `FRESH_S`=300 秒内就用它渲染（`read_latest()`/`fmt_table_v2()`，表头带最后采样时刻，判定/进度/速率/token/ETA 全部现成），过期打一行警告退回现有的现场实探 `collect()` 老路；json 出口过期时给老 `collect()` 结果外加 `sampler_stale: true` | `python3 run.py gpu-jobs register --name N --workdir W --piece host:gpus:session:logpath …`；`… free`；`… finish NAME [--force]`；⚠️ watch 对 shiga 误报 EXIT（ssh host key）；`NEW1_MONITOR_DIR` 环境变量可把采样历史读取目录指到别处（单测用） |
