...
| 评测端超长事件默认处理（2026-08-28 起） | `--overlong` 三个评测脚本默认 `left`（提示/全文超长照旧左截，行为与加这个开关之前逐字节不变）；`skip`/`drop-event` 换算口径要显式传 | `eval_causal_call.py`/`eval_causal_param.py`/`eval_tool.py`（spec 16.2，工单 07） | 换成非默认值会改变进分母的行/事件集合，矩阵只收 `left` 的评测结果（spec 16.10 #35），其余模式的数只进各自报告 |
