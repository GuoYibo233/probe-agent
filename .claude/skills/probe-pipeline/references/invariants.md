...
| causal 超参 | lr 1e-5（LoRA 走 `lora_util.resolve_lr`，默认 2e-4）/ warmup 5% / clip 1.0 / fp32 + bf16 autocast，三格不变。**一次更新统一 8 个事件，但 bs/accum 的拆法与 epochs 数 2026-08-28 起分家**（换实现，spec 第 1 节）：ctool `--bs 2 --accum 4`（决定 15：8192 上限下 `--bs 4` 训练首批在 H100 OOM——进程 92.94 GiB，退到 `--bs 2`，`ops/runs.jsonl` 的 `ks828b06_gptoss_ctool_h100mem_bs2`），`--epochs` 仍 3；cgen/cparam（`train_causal_share.py`）`--events-per-mb 4 --accum 2`，`--epochs` 改 1（理由是 np821 的 8 个 cgen/cparam run 的 val_ce 全在 epoch 0 最低，而 4 个 ctool run 三个 epoch 一路升，`plans/2026-08-28-plan.md` 第 12.7、12.8 节，草稿 2.5；新训练器每个 epoch 同样把每个切点过一遍，只是前缀共享算得快，epoch 数和算力无关）。三格更新单位仍是 8 个事件，"同一骨架多个头"的对照没有被打破。**np821 批用的是 bs 4 事件 / accum 8（32 个事件一次更新）/ epochs 3**，三格当时同一套超参 | `train_causal_tool.py`（工单 02）、`pipeline/train/train_causal_share.py`（cgen/cparam 现役，工单 03）；旧逐行脚本 `train_causal_callgen.py`、`train_causal_param.py` 冻结为参照，经 `train-cgen-rows`/`train-cparam-rows` 发射，仍是 np821 那套口径不变（ENG §5.3、§5.4） | 同上；且 cgen/cparam 的超参是**故意对齐 ctool** 的，动一个就破坏"同一骨架多个头"的对照（cgen 对 cparam 的比较更是同批样本同答案只差拼串，超参一分家差距就没法归因到"工具名是否给定"）；ctool 与 cgen/cparam 的 epochs 数不再相同这条本身也要留意——三格互比时"训了几遍数据"已经不是同一个数 |
| 左截断 | `truncation_side="left"`，`padding_side="right"`，pad=eos | 照抄 `train_causal_probe.py` 的 `build()`（ENG §5.4） | 右截会吃掉紧邻调用的思考尾巴，正是探针最该看的那一段。**cgen/cparam 现役训练器 2026-08-28 起不适用这条**——它不截断任何输入，见下面「上限」行 |
| 上限 `--max-len`（三格统一） | 默认 `8192` 不退档（2026-08-28 冒烟裁决），事件全文 token 数超过就**整条丢弃**（不是逐行截断），计数进 `dropped_events_train`/`dropped_events_val`（ctool）或 `start` 事件的 `dropped_events`（cgen/cparam）；三格共用同一个上限，训练事件集才不分叉（spec §15）。依据：`ks828b06_gptoss_cgen_speed_b16k`（H100）训练 step 峰值 60.59 GB，对 93.10 GiB 余量 39%；ctool smoke 在 8192 上限、`--bs 2` 下 nvidia-smi 最大样本 56,859 MiB（`pipeline/runs/smoke/ks828b06_*`、`ops/runs.jsonl`） | `pipeline/train/share_data.py` 的 `load_events`（cgen/cparam，工单 01）、`train_causal_tool.py` 的 `load_events`（ctool，工单 02） | 上限变了，三格训练事件集与丢弃比例一起变，与新口径之前的 `dropped` 统计不可比；三格必须同步改，否则某格看到的事件集比另一格窄。**np821 批用的是 `max_length=4096`，逐行左截而不是整条丢弃** |
| cgen 目标构造 | 目标串构造不变：先 tokenize 目标不截断（>160 token 的实例丢弃并计数 `dropped_rows_tgt`），`labels` 前缀段填 `-100`。**输入侧左截规则 2026-08-28 起只在冻结的旧逐行脚本（`train_causal_callgen.py`，经 `train-cgen-rows` 发射）里还生效**：按 `max_length = 4096 - L_t` 左截输入；**cgen/cparam 现役训练器不左截**——事件全文一次前向，公共前缀之外的目标段直接拼接在后面，整个事件超过「上限」行的 `--max-len` 才整条丢弃（不是按单行截断） | `train_causal_callgen.py`（旧口径冻结，ENG §5.4）；`pipeline/train/share_data.py` 的 `load_events`/`pack_event` 与 `train_causal_share.py`（现役，工单 01/03） | 旧脚本顺序反了就会被左截吃掉目标，损失算在错的位置上（这条只对冻结的参照脚本仍然成立）；现役训练器不截断，风险变成「`--max-len` 定太小会把长事件整条丢光，样本量骤降」而不是「截断损坏目标」。**np821 批用的是 `max_length = 4096 - L_t` 左截输入** |
...
| cgen/cparam 选 best | 唯一依据是 val 全量 masked-CE（越低越好）；`val_exact_call` / `val_exact_params` 只进日志不选 best。**现役训练器（`train_causal_share.py`，2026-08-28 起）这项生成式评估走开关**：`--gen-eval` 默认 200 行（0 关闭）、`--gen-eval-at` 默认 `last`（只在 epoch 末那次评估做生成），决定 28 定的默认值就是"和旧口径同规模、只进日志不选 best" | ENG §5.4；cparam 2026-08-21 同口径；`--gen-eval`/`--gen-eval-at` 默认值见 spec 16.3（工单 08） | 用 exact 指标选 best 会让 val 被用两次（选点 + 报数），破坏 val/test 分工；把 `--gen-eval-at` 从 `last` 改成 `all` 只是多做几次生成、不进 best 判据，不算破口径 |
...
| smoke 规模 | mtool / mext（停跑）：**按各脚本自己的 `SEED` 常量随机抽**（20260729）500 训练 / 200 评估**实例**（mext 的这两个数落在展开后的**参数实例**上，不是样本）；ctool：同法随机抽（`SEED`=42）200 / 80 **事件**（它按事件计数，不是实例）；**cgen/cparam 现役训练器（`train_causal_share.py`）2026-08-28 起换成确定性抽样**：`--smoke` 默认 40 训练事件 / 16 评估事件，**按事件全文 token 数升序取前 N 个**（train 与 val 同规则，不依赖 `SEED`，不是随机抽）；旧逐行脚本（经 `train-cgen-rows`/`train-cparam-rows`）仍是随机抽 500/200 实例的旧口径。各格/各口径都是 1 epoch | ENG §5.1、§5.4（mtool/mext/ctool 与旧逐行脚本，`random.Random(SEED).shuffle(...)` 之后再切前 N 条，**不是原序截断**）；`share_data.load_events(order="shortest")`（cgen/cparam 现役，工单 01/03） | 只影响冒烟，但改了就失去"和历史 smoke 同口径"的对照价值；改种子或改成原序截断都会换掉抽中的那批样本。**cgen/cparam 换成确定性升序抽样后，新旧两代 smoke 数字不可比**——重跑同一个 `--smoke` 会拿到完全相同的事件集合，但不再是"和历史 smoke 同口径"的对照 |
| 显存探针挑块方式默认值（2026-08-28 起） | `--mem-probe-pick` 默认 `cost`（枚举本次 run 抽样出的事件的物理块，按 token 数最大/损失位数最大/归一化和最大挑三块取峰值大者）；`tokens`/`loop` 仍在，换挡属可调旋钮 | `train_causal_share.py`（spec 16.5，工单 10） | 只影响探针挑哪块量峰值，不进梯度；换默认值要记 TIMELINE，免得后面拿"探针挑的块不一样"当成真实显存变了 |
...
| 对齐判据默认值（2026-08-28 起） | `--align-rule` 默认 `abs`（逐行/逐 token 绝对差，现状）；cgen/cparam 的四个粗筛门槛（`--align-tok-tol`/`--align-bf16-mean-tol`/`--align-bf16-max-tol`/`--align-baseline-factor`）默认值就是第一轮写死的那几个常量（`3e-4`/`2e-2`/`1e-1`/`3.0`）；`--align-rel-tol` 默认 `1e-5`（ctool 与 cgen/cparam 都是） | `train_causal_share.py`/`train_causal_tool.py`（spec 16.4，工单 09） | 换 `--align-rule` 或调门槛属可调旋钮（invariants §6 的 `--align-tol` 行同款判据），换了要记 TIMELINE；默认值不改就是"和第一轮同一把尺子" |
...
| 评测端超长事件默认处理（2026-08-28 起） | `--overlong` 三个评测脚本默认 `left`（提示/全文超长照旧左截，行为与加这个开关之前逐字节不变）；`skip`/`drop-event` 换算口径要显式传 | `eval_causal_call.py`/`eval_causal_param.py`/`eval_tool.py`（spec 16.2，工单 07） | 换成非默认值会改变进分母的行/事件集合，矩阵只收 `left` 的评测结果（extending §5 #35），其余模式的数只进各自报告 |
