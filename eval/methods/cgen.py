"""Exact match of the generated call against the target at the frozen classifier theta.
...
def match(pred: str, target: str, env) -> dict[str, bool]:
    ...
    if t is None:
        raise ValueError(f"cgen.match: target {target!r} does not parse via env.split_args")
    t_tool, t_args, _ = t
...
def report(pred_df: pl.DataFrame, cfg, ref, labels: list[str] | None) -> tuple[dict, None]:
    ...
    if not (isinstance(ref, tuple) and len(ref) == 2):
        raise ValueError(f"cgen.report: ref must be a (fields, fires) tuple, got {type(ref).__name__}")
    ref_fields, ref_fires = ref
    if labels is not None:
        raise ValueError(f"cgen.report: labels must be None for a generator method, got {labels!r}")
    ...
    for risk in cfg.eval.risk:
        ...
        n = joined.height
        results = [match(row["text_pred"], row["target"], env) for row in joined.iter_rows(named=True)]
        tool_ok_vals = [r["tool_ok"] for r in results]
        params_vals = [r["params_all_ok"] for r in results]
        full_vals = [r["full_call_ok"] for r in results]
