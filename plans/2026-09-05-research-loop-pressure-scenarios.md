...
表里 deploy 那一格有一处用错了 kind（评审核出来的）：deploy 给 gyb「说配置里两个键训练器不读」用的是 `fyi`，而 03 第 90 行定的 `fyi` 是「gyb 越过 owner 处理了你的单子」的通知类，04 第九节说三种通知都是 rl 在触发点自己开的行，不是角色拿来告诉 gyb 一件事的口子；这里该开的是 `request` 或者 `anomaly`。rl 没拒是因为 issue.py 不按角色限 kind；统筹已裁成 D-38：三种通知类 kind 只由 rl 开，手开退出码 2，由 issue.py 拦，说明书不另写句子，D-38 落地之后 rl 会拒。
