...
- `rl issue open/reply/reassign/close/link/show/list` 的完整参数，`rl inbox` 列哪五类，`rl status` 的十段，`rl doctor` 的扫描项：05-rl-cli.md；gyb 怎么看到等他的事（桌面通知与 `rl notify` 2026-08-21 裁掉不做，只剩 `rl status`）：01-gyb.md 第五节（2026-08-17 gyb 裁）。
