...
| gyb 的身份规矩（裸终端就是 gyb、actor 判定、`--as-gyb` 加 `--quote`、`--force --reason`、豁免范围、grants 只收裸终端——2026-08-17 gyb 裁这一组定义处归 `01`，`05`「actor 怎么定」是命令行写法算写了两遍）、gyb use case 表、`rl status` 十段、推送表、定期提醒 | `01-gyb.md` | 全部除 `08`（原表只列 `05`、`04`、`14`、`25`；核下来其余十六份的接口一节都指了它） |
...
- 推送表和 `rl notify`：`07`、`10`、`11`、`20`、`25` 把推送表指到 `01`，`09`、`22`、`23`、`24` 指到 `05`。原表把推送表归 `01`、notify 归 `05`。
- 阈值表：见 `08` 那一行。
