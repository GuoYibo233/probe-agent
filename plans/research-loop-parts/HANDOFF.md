...
| gyb 的身份规矩（裸终端就是 gyb、`--as-gyb` 加 `--quote`、豁免范围、grants 只收裸终端）、gyb use case 表、`rl status` 十段、推送表、定期提醒 | `01-gyb.md` | 全部除 `08`（原表只列 `05`、`04`、`14`、`25`；核下来其余十六份的接口一节都指了它） |
...
2. 上表里标了「写了两遍」的五处（`03` 对 `09`/`07`/`04`、`05` 对 `04`/`02`/`07`/`09`、`06` 对五份角色、`06` 对 `08`、`01` 对 `05` 的 `rl status` 十段——README 里 `01` 和 `05` 的覆盖栏都写着 status 十段），每次同步结束都要各对一遍，两边一字不差。
...
- actor 判定、`--as-gyb` 加 `--quote`、`--force --reason`：`02`、`10` 指 `01`，`03`、`06` 指 `05`，`04`、`14`、`22` 指 `01` 和 `06`。原表把「gyb 的身份规矩」归 `01`、命令签名归 `05`、钩子归 `06`，按这个分法三处各占一段。
