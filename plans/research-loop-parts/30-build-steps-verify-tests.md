...
|---|---|---|---|---|---|
| 1 | Bash 环境里有没有现成的会话 id 变量 | 在会话里 `env \| grep -i session`，再对比钩子输入的 `session_id` | 同一个值 | SessionStart 钩子写状态文件（`loop/.sessions/<session_id>.json`），rl 按 `cwd` 加最近一次登记找 | 已测 2026-09-05（顺带，验证助手，`plans/2026-09-05-research-loop-verify.md` 2.7 节）：Bash 里有 `CLAUDE_CODE_SESSION_ID`，值与同一会话钩子输入的 `session_id` 逐字相同，主案成立，备案不用；状态文件仍按 `06` 放 `loop/.sessions/`。此前 2026-08-18 一度并入「`${CLAUDE_PLUGIN_DATA}` 解析到哪」半条，同日撤销 |
...
| 5 | SessionEnd 和 SubagentStop 在 subagent 结束时触发不触发、会话 id 是不是同一个 | 起一个加载角色的 subagent，让它写一行账，结束后查 sessions 账 | 有 `ended_at`、`session_id` 和 `started_at` 那行相同 | 全靠 `rl status` 段 7 加 `rl reclaim`，提醒周期从 7 天缩到 1 天 | 已测 2026-09-05（验证助手，`plans/2026-09-05-research-loop-verify.md` 第二节）：SubagentStart、SubagentStop 都触发并带 `agent_id`、`agent_type`，`session_id` 与母会话相同；SessionEnd 在子会话结束时不触发；子会话 Bash 环境与母会话逐字相同，rl 按状态文件读到的是母会话的角色（`06` 第 118 行那句成立）；另立信号里环境文件那条不通、PreToolUse 钩子用 updatedInput 往命令前注入变量通。处置按代裁 D-15（`plans/2026-09-04-research-loop-proxy-decisions.md`）：rl 先看注入的 `RL_AGENT_TYPE`、`RL_AGENT_ID`，子会话在 sessions 账另落一行带 `agent_id`，SubagentStop、SessionEnd 各自交回；`04` 第六、七节的落点记 sync-inbox 问题 49，等最后一期 |
...
| 9 | 后台 subagent（原则 11）：父会话活着时后台 subagent 能不能跑几个小时；父会话结束后台 subagent 会不会被杀 | 起 deploy 会话后台起一个 sleep 两小时的 run subagent，两种情况各试一次 | 活着时能跑完并回通知；父会话结束时的行为有结论 | 被杀的话正案不变（GPU 在 tmux、单子在账上、下一个 run 认领），只是待认领的单子多；同步等的老方案不再回来 | 已测 2026-09-05（验证助手，`plans/2026-09-05-research-loop-verify.md` 第三节）：母会话活着时后台子会话跑满 1200 秒并回通知，主案成立（几个小时没测，1200 秒内没碰到任何超时）；母会话三种结束方式（`/exit` 选退出、关终端、挪到后台）子会话都被杀，失败备案照旧成立；打印模式的母会话默认只等后台子会话 600 秒，设 `CLAUDE_CODE_PRINT_BG_WAIT_CEILING_MS=0` 才等到底（代裁 D-23 写进说明书）；「挪到后台」保住的是会话不是子会话，新 id 是裸会话（代裁 D-20） |
| 10 | 钩子输入里有没有模型标识 | 打印 SessionStart 钩子的输入 JSON | 有 model 字段 | sessions.model 记 `unknown`，doctor 列出来，gyb 事后补 | 备案已收成正案的一部分（2026-08-17 随 `05` 定稿裁）：doctor 第 19 项扫 `model` 是 `unknown` 的 sessions 行，修法 `rl session amend ID --model M`（`05`「rl doctor」、`04` 第七节）。这一条照测，测出有 model 字段就少走一次 amend，不改正文。顺带看到的（2026-08-18，不算正式测）：一次 PreToolUse 观察里钩子输入有会话 id、工作目录、权限模式、工具入参，没有模型标识。已测 2026-09-05（顺带，验证助手，`plans/2026-09-05-research-loop-verify.md` 2.7 节）：交互会话的 SessionStart 输入有 `model` 字段，打印模式的没有，所以「取不到记 `unknown`」和 doctor 第 19 项留着 |
...
原文：对每一行合法转移各一个用例，加三个非法转移用例（跨状态、错角色、缺前提），非法一律退出码 2 并且账里没有新行（补注 2026-09-05 施工会话：错角色撞的是前提栏（`04` 第 63 行「写入会话的角色等于 `to_role`」）退出码 2，撞的是「谁能写」栏（`04` 第 70 行 owner 或 gyb 那一档）按 `03` 第 27 行是权限层、退出码 3（`03` 第 220 行），原文照抄不删）；`holder` 只在 `in_progress` 非空，进写、出清、`last_holder` 留下；start 时 holder 非空拒收；认领用例（最新尝试有 launched 未 finished 的 run 行时 start 标 adopted）。
...
原文：空列表拒收；三类来源各一个用例；`file` 类路径不存在拒收、带锚点通过；`run` 类 run_id 不在 runs 账拒收；update 不给 source 继承上一版；confirm 正文不变版本加一；merge 自动并入被合并编号并按 `--root` 记根；`decisions.gyb.jsonl` 拒收非 cli 行（已裁：`02` 第 11 行、sync-inbox 问题 34(e)，只装裸终端新开的决定和后续版本，只有 add 版要 `session_id` 是 cli、后续版可来自角色会话；2026-09-05 施工会话补注，原文照抄不删）；update 时打印引旧版的活单。
...
- `decisions.gyb.jsonl` 只收 `session_id` 是 `cli` 的行这一条不变，角色会话里 `--as-gyb` 的决定落角色那本、actor 记 gyb（`02`、`05`），测试 3 已有一条，这里不重复。（已裁：`02` 第 11 行、sync-inbox 问题 34(e)——只有 add 版要 cli，后续版可来自角色会话；2026-09-05 施工会话补注，原文照抄不删。）
...
要加的：`verdict_text` 在 `accepted` 和 `rejected` 两版都必填（`03` 裁决记录）；accept 列出还活着的会话和它们的 `rules_version`、打印待办（`09` 第四节、`05` 命令表）；accept 把 `rules_version` 加一并写回 `common/GLOBAL-RULES.md` 头部那一行（`rules_version` 是整数、从 1 起、那一行是唯一真源，2026-08-18 `09` 定稿裁）；`feedback accept --text` 必带（sync-inbox 第五段）。
...
- 2026-09-05 施工会话（gyb 2026-09-04 点名今天测第 5、9 条；验证助手实测记录 `plans/2026-09-05-research-loop-verify.md` 06becf4、c88074f）：待验证第 5、9 条状态栏改已测，第 1、10 条顺带测到的结果照记；第 5 条的处置代裁 D-15、第 9 条连带 D-20、D-23，冻结正文的落点记 sync-inbox 问题 49。
- 2026-09-05 施工会话（评审助手核测试 9d47481 时指出，三处都是传播已有裁决）：测试 2 原文「非法一律退出码 2」补注错角色撞前提栏退 2、撞谁能写栏退 3（`03` 第 27、220 行，`04` 第 63、70 行）；测试 3 原文与测试 9 那句「`decisions.gyb.jsonl` 拒收非 cli 行 / 只收 cli 行不变」补注已被 `02` 第 11 行和 sync-inbox 问题 34(e) 取代（只有 add 版要 cli），原文照抄不删。
