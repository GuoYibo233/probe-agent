...
> 这份覆盖 research-loop 插件的目标与总验收、十一条设计原则的全文（带第二轮补的推论）、施工前的九条裁决、第六轮改了但 gyb 还没裁的九条、审读意见里没采纳的九条、讨论中改掉的十六条方向、两轮模拟的说明与原始文件路径、这套拆分文档的索引。
...
总验收由 gyb 定：每个角色配一个对应的任务，两个 agent 一个加载角色 skill、一个不加载，各做一遍，产出摆在一起由 gyb 自己看，不预先定判分办法。五个任务留到验收前由 gyb 定。
...
十一条原则要抄一份在 `research-loop/ARCHITECTURE.md` 的开头，并进公共母版，母版里的编号是 principle-01 到 principle-11。母版本身见 `09-common-and-feedback.md`。
...
## 四、第六轮改了、gyb 还没裁的九条（a）到（i）

第二轮模拟之后写施工计划的人改的设计变动，一行一条，gyb 不认就改回。

| 编号 | 改了什么 |
|---|---|
| （a） | 派活从同步等改成后台派加认领（原则 11） |
| （b） | gyb 的豁免收窄成只豁免权限、不豁免完整性，硬写用 `--force --reason` |
| （c） | `--as-gyb` 在角色会话里一律要 `--quote`，gyb 本人敲也要 |
| （d） | grants 只收裸终端 |
| （e） | 分析单开单时口径可以是 proposed，交活才要全 approved（改了设计文档「idea 也可以给 analysis 开单，但是单子里只能引已经批准的口径行」那一句） |
| （f） | 快车道加 `rl ql open/close`、补单只要一份简报、验收人固定 gyb、analysis 也能走 |
| （g） | N 张发射单由一个 run 会话接整个 batch |
| （h） | issue 加 `failed`、`orphaned`、`fyi` 三种 kind |
| （i） | `rl inbox`、`rl trace`、`rl handoff amend/resume/reissue`、`rl decision confirm`、`rl session show/list/focus` 几条新命令 |

这九条已经按新写法写进两份源文档的正文，裁决还没做。
...
两处原文不一致：第 4 条的正文说「全量搬保留，走 workflow」，括号里又注明当晚改裁作废；施工计划裁决 2 直接写入口 skill 不做全量搬迁的 workflow。按裁决 2 为准，老代码由 gyb 手动按需搬。
...
这一版按十一条原则砍掉的手续：快车道出口一份简报、同 batch 一次 smoke、后台派活不占终端、认领代替重跑、amend 代替收旧单开新单、reissue 一条命令、一句话批一组口径、init 时给 read:notes、accept 自动关 issue、reclaim 自带修法命令。剩下多少步没有再数，第三轮模拟再数。第三轮跑不跑由 gyb 定；跑的话走同样十七个场景，重点数手续量、验第一节（a）到（i）九条改动有没有引出新的打架，跑之前先把 new1 第一次 rl init 那个没跑成的场景补上。

## 八、施工步骤一览

施工之前 gyb 先自己打磨设计文档。之后的八步，每步的交付物、验收、依赖、执行者与模型在施工计划第十一节，展开在 `30-build-steps-verify-tests.md`，这里只列步骤名：步 0 验证（第九节 11 条待验证）、步 1 清空旧代码、步 2 架构文档、步 3 共同底座、步 4 交流机制、步 5 公共母版、步 6 五个 SKILL.md、步 7 最小一条路、步 8 总验收。
...
| 文件 | 一句话 |
|---|---|
| `00-overview.md` | 本文件：目标与总验收、十一条原则、九条裁决、（a）到（i）九条待裁、审读没采纳九条、改掉的方向十六条、两轮模拟说明、文档索引 |
...
| `08-trees-init-and-host.md` | 研究仓库和插件本体两棵树、`rl init` 建什么、入口 skill、宿主台账对接 |
| `09-common-and-feedback.md` | 公共母版（公共规矩八条、词表、五栏规格、读法）和 feedback 账 |
...
- actor 的取值（五个角色或 `gyb`）、`--as-gyb` 与 `--quote` 的规矩：定义在 `01-gyb.md` 和 `05-rl-cli.md`，原则 1 用它。
...
- 九本账的公共骨架（`id`、`version`、`status`、`ts`、`actor`、`session_id`、`schema_version`、`fix_for`、`force_reason`）：定义在 `03-ledgers.md`，原则 4 用它。
...
- `rl ql open`、`rl ql close`、`ql_tag`：定义在 `07-quick-lane.md`，原则 7 用它。
- `parent_id`、`root_id`、`batch`、`attempt`、`rl trace`：定义在 `02-decisions.md`、`03-ledgers.md`、`05-rl-cli.md`，原则 9 和原则 10 用它。
- 公共母版里的 principle-01 到 principle-11 编号和 rules_version：定义在 `09-common-and-feedback.md`，第二节那十一条抄进去。
- 待验证十一条的题目和施工八步的细节：在 `30-build-steps-verify-tests.md`，第八节只列步骤名。
- gyb 手动搬老代码、入口 skill 的迁移提醒（裁决 2）：在 `08-trees-init-and-host.md`。

## 源文档没写清的（留给 gyb）

1. 这套 parts 文档和 `research-loop/ARCHITECTURE.md`（施工步 2 的交付物，要求两棵树每个文件一行加十一条原则抄一份）是什么关系，源文档没写：parts 是不是 ARCHITECTURE.md 的底稿，还是两份并存。
2. parts 文档和两份源文档谁为准没写。原则 8 只裁了「施工计划第一节的裁决优先于设计文档正文」，没说拆出来的 part 文件在什么位置。
3. （a）到（i）九条如果 gyb 否掉其中某条，正文哪几行要回退，源文档没有回退清单，只说「gyb 不认就改回」。
4. 十一条原则彼此打架的时候按谁，源文档没写。已经出过一次：原则 1 的 gyb 豁免和原则 4 的完整性校验靠（b）那条待裁改动收窄，（b）如果被否，两条原则就没有裁决线。
5. 总验收的五个任务没定，判分办法明说不预先定，产出摆在一起之后按什么算通过也就没有落点。
6. 第三轮模拟跑不跑没定；这一版砍掉手续之后剩多少步没有再数。
7. 十一条原则里只有八条在第一轮时对应了「改了正文哪一处」，第九、十、十一三条是第二轮新增，源文档没有它们在第一轮语境下的对应条目，这一份也没有补。
