...
> 这份不覆盖任何机制细节：五个角色各自干什么在 `10-role-idea.md`、`11-role-deploy.md`、`12-role-run.md`、`13-role-analysis.md`、`14-role-reviewer.md`；七本账的行格式在 `03-ledgers.md`（decisions 在 `02`，handoffs 在 `04`）；派活单转移表和会话生命周期在 `04-handoffs-and-sessions.md`；`bin/rl` 命令表在 `05-rl-cli.md`；分权三层和钩子在 `06-hooks-and-permissions.md`；快车道在 `07-quick-lane.md`；两棵树、init、宿主对接在 `08-trees-init-and-host.md`；待验证清单、测试清单、施工步骤在 `30-build-steps-verify-tests.md`。
...
第一轮模拟报了 59 条摩擦，归并之后根子只有八个。第二轮模拟按八条改过的文档再走十七个场景，报了 234 条（未核实），归并之后：八条原则本身没有一条被推翻，但是有五条只写了正面一句、没把推论写出来，模拟者在推论上各猜各的（2026-08-21 评审补注：定稿正文里带『第二轮补的推论』标记的是七条，原则 7 没有；这句历史计数与正文对不上，以正文为准）；另外有三个根子八条里没有。下面十一条是根子的正面写法，带着推论；正文里每一处规矩都要能对回其中一条，施工时遇到文档没写到的岔路也按这十一条推。
...
6. 进出对称，等人的事有收件箱。每本账能写进去就能查出来。第二轮补的推论：收件箱有两个，gyb 的叫 `rl status`，角色的叫 `rl inbox`。任何一版把某一行送进「等某人」的状态（等 gyb 批、等 owner 验收、等 assignee 回、等 owner 拉起、等谁读通知），那一行必须出现在那个人的收件箱里；桌面通知这一版不做（2026-08-21 gyb 裁，原话「收件箱这个算了 先不做，就维护一个我要看的东西就行，我自己记得定期手动看」），gyb 的收件箱只有 `rl status` 一个出口，gyb 自己定期手动看。`rl inbox` 是查询命令，不是上线动作：角色被拉起不自动查收件箱，先干拉它起来的那张单，`rl inbox` 谁需要谁敲。
...
2. 「读 transcript 认角色」这条验证路线废弃，被 skill 头部钩子取代。（skill 头部钩子这个挂法 2026-08-18 又作废了，现行是插件级钩子文件按 agent_type 判角色，见 06）
3. 「一个决定更新就换新编号加 superseded_by」废弃，被「编号不变加版本号」取代，理由是下层引用不会挂在旧版上。
4. 「决定账一个字段不拆文件」的旧倾向被 gyb 推翻，拆成五个文件（2026-08-16 晚加 gyb 一个文件，成六个）。
5. reviewer 不管文献这条上午锁过，下午 gyb 把整条文献线说全了：文献变 idea 只有 gyb 亲自做，idea 读文献要经允许。（获准机制 2026-08-21 砍掉：idea 读 notes/ 不用获准，只留『文献变想法 gyb 亲自做』的纪律，见 10）
...
14. 「通知走 loop/inbox 加 SessionStart 播报」这个提议被 gyb 改成桌面弹窗加 rl status。（2026-08-16 夜补：角色那一头加 `rl inbox`，是查询命令不是播报。）（桌面弹窗 2026-08-21 又裁掉不做，等 gyb 的事只剩 rl status 一个出口，见 01）
...
第二轮：十六个 opus subagent 各走一个场景（十七个里 new1 第一次 rl init 那个没跑成），报了 234 条摩擦；十七个核实者和 critic 因为月度用量上限没跑成，所以这 234 条没有经过对抗核实，写设计文档的人自己对着两份文档核了每一堆里引用最多的那几条，核过的都成立。234 条里 107 条是 missing、44 条 contradiction、30 条 ambiguous、22 条 too_heavy、17 条 principle_violation、8 条 blocked、6 条 guessed；40 条标 blocks。归堆之后是十一堆，前八堆对应第五轮的八条原则各缺一段推论（2026-08-21 评审补注：定稿正文里带『第二轮补的推论』标记的是七条，原则 7 没有；这句历史计数与正文对不上，以正文为准）、后三堆是新根子。每一堆归了哪些问题、改了两份文档的哪里，写在设计文档最后一节和施工计划第十四节，这份不抄。
...
这一版按十一条原则砍掉的手续：快车道出口一份简报、同 batch 一次 smoke、后台派活不占终端、认领代替重跑、amend 代替收旧单开新单、reissue 一条命令、一句话批一组口径、idea 读 notes/ 免获准（获准机制 2026-08-21 整套砍掉）、accept 自动关 issue、reclaim 自带修法命令。剩下多少步没有再数。第三轮模拟先不跑，等整套拆分文档定稿、施工完成之后再说（2026-08-18 gyb 裁）；到时跑的话走同样十七个场景，重点数手续量，跑之前先把 new1 第一次 rl init 那个没跑成的场景补上。
...
| `03-ledgers.md` | 七本账的行格式（decisions 指 `02`、handoffs 指 `04`）、每本的 status 取值、必填规则、公共骨架、锁与写序 |
...
| `09-common-and-feedback.md` | 公共母版（公共规矩九条、词表、五栏规格、读法、判断类检查清单）和 feedback 账 |
...
- `parent_id`、`root_id`、`batch`、`attempt`、`rl trace`：定义在 `02-decisions.md`（`root_id`）、`04-handoffs-and-sessions.md`（`parent_id`、`batch`、`attempt`，handoffs 的行格式定义处是 `04`；runs 账自己的 `attempt` 栏在 `03`）、`05-rl-cli.md`（`rl trace`），原则 9 和原则 10 用它。
...
- 2026-08-21 来自 `01-gyb.md` 定稿（`cd569ab`，rl-hub-v5 传；gyb 原话「收件箱这个算了 先不做，就维护一个我要看的东西就行，我自己记得定期手动看」「那这个砍了吧」）：原则 6 推论里「桌面通知只是 gyb 收件箱里几段的推送，哪几段推送写成一张表」那半句按「桌面通知不做」改掉。对回原则 6。
- 2026-08-21 来自 `07-quick-lane.md` 定稿（`7a01842`，rl-hub-v5 传）：索引 `09` 行公共规矩八条改九条（rule-09 2026-08-21 gyb 立）。
- 2026-08-21 来自 `10-role-idea.md` 定稿（`96459b4`，rl-hub-v6 传；gyb 原话「砍掉，默认能读」「不用申请」）：第七节砍掉手续清单里「init 时给 read:notes」改成「idea 读 notes/ 免获准」（获准机制整套砍掉）。对回原则 2。
- 2026-08-21 评审修复（gyb 授权；定义处本份第二节）：第二节导语「有五条只写了正面一句、没把推论写出来」半句后加括号补注，写明定稿正文里带「第二轮补的推论」标记的是七条、原则 7 没有、以正文为准；历史句本身不改。对回原则 8。
- 2026-08-21 评审修复（gyb 授权；定义处本份第二节）：第七节第二轮那段「前八堆对应第五轮的八条原则各缺一段推论」后加同一句括号补注（七条，原则 7 没有，以正文为准）；历史句本身不改。对回原则 8。
- 2026-08-21 评审修复（gyb 授权；定义处 `06`）：第六节第 2 条「被 skill 头部钩子取代」后加尾注，skill 头部钩子这个挂法 2026-08-18 作废，现行是插件级钩子文件按 agent_type 判角色；原条目不动。对回原则 8。
- 2026-08-21 评审修复（gyb 授权；定义处 `10`）：第六节第 5 条「idea 读文献要经允许」后加尾注，获准机制 2026-08-21 砍掉，只留「文献变想法 gyb 亲自做」的纪律；原条目不动。对回原则 8。
- 2026-08-21 评审修复（gyb 授权；定义处 `01`）：第六节第 14 条「改成桌面弹窗加 rl status」后加尾注，桌面弹窗 2026-08-21 裁掉不做，等 gyb 的事只剩 `rl status` 一个出口；原条目不动。对回原则 6。
- 2026-08-21 评审修复（gyb 授权；定义处 `04`）：接口一节 `parent_id`、`batch`、`attempt` 三个 handoffs 字段的指向由 `03-ledgers.md` 改指 `04-handoffs-and-sessions.md`，`root_id` 仍指 `02`、`rl trace` 仍指 `05`。对回原则 8。
- 2026-08-21 评审修复（gyb 授权；定义处 `03`）：第九节索引 `03` 那行「九本账的行格式」改成「七本账的行格式（decisions 指 `02`、handoffs 指 `04`）」。对回原则 8。
