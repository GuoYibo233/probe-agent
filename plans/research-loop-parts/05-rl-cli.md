...
查询命令是 show、list、trace、status、inbox、stale、doctor 七类，谁都能调，不进角色 json 的 `ledger_writes`。这是原则 2 的推论：读一律不设权，角色 json 的 `reads` 栏是纪律不是门禁。机器检查只查 SKILL.md 里出现的写命令在不在这个角色的 `ledger_writes` 里。

所有角色上线的第一个动作是 `rl inbox`。
