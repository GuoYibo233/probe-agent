...
- 问题 31（2026-08-17，来自 03 落地）：runs 的 `relink` 版（doctor 第 6 项修法，只改 `handoff_id`）`status` 填什么——03 总规矩说校验按 `status` 查，runs 只给了 `launched`/`finished`/`adopted` 三个值。（a）照 sessions 的 `amend` 版：`status` 与其他栏照抄最新版、只换 `handoff_id`；（b）另加一个 `relink` 值。——状态：等 gyb。裁了改 03 runs 一段、`05:73`「三版」那句。
- 状态：已处理 2026-08-17（04 `a7d1ec9`+`cec2cc9`+`9b78d7c`、05 `1b37593`+`2c965d9`+`2dcdd23`+`ddafd84`、03 `81377ac`，底座三份这一轮齐了；其余各份、两份源文档按 HANDOFF 第八节由统筹落；问题 31 等 gyb）
