...
- 要改的地方：
  48. 冻结后待议：（a）`03:145` feedback 行格式 `id` 一栏「反馈编号」补形状「形如 `fb-0001`」（D-11，照 `iss-0031` 类推）；（b）`04` handoffs 字段表 `work_order` 顶层 `track` 一栏（问题 45(a)(f) 加的）标「开单时必填」，转移表「（新建）→ todo」行 `work_order` 的前提加 `track`（D-10，依据 `04:43` 发射单第一次尝试 `track` 必填、`11:76` 发射单从父单抄）；（c）`03:196` scratch 段「actor 是 deploy 或 analysis」补「gyb 在裸终端或 `--as-gyb` 写照收（`01:65`、`03:27`）」（D-16）；（d）`05:65` `rl handoff start ID [--batch B]` 签名旁补「带 `--batch` 时 ID 只用来定位 batch（必须属于 B），同 batch 全部 todo 的发射单一起置 in_progress、holder 同一会话；host 和 gpus 怎么分仍等 `21` 留给 gyb 第 6 条」（D-22）；（e）`03:15`「closed 会话再写拒收」补半句「`rl session start` 除外：重新加载角色的登记写的是同一个 `session_id` 的下一版 open，别的写命令照旧按最新版查」（D-28；`04:152` 同句）；（f）`05:65` `rl handoff done ID [--notebook P --figure P ...]` 的省略号展开成「`work_order` 用 `--report-method P --report-detail P --code-path P`，`analysis_order` 用 `--notebook P --figure P`」（D-32）；`rl handoff amend ID` 补「不带 `--command` 时抄上一次尝试追加一版」（D-31）；（g）`05:136` inbox 第 2 项的状态集合写明「todo、stuck、rejected、done_pending_review」（D-33）；（h）`05:206` doctor 第 12 项补「通知类 issue（withdrawn、orphaned、fyi）不算」（D-34）；（i）`05:159` status 段 5 补「只列 todo」（D-35；`01:108` 同句归 01）；（j）`05` 九本账 `show` 的签名加 `[--history]`（打印全部版本，默认最新版；D-37）；（k）`03:90` 三种通知类 kind（withdrawn、orphaned、fyi）补「只由 rl 在触发点开，手开退出码 2」（D-38）。
- 状态：等最后一期（代裁 2026-09-05，gyb 审 D-10、D-11、D-16、D-22、D-28、D-31 到 D-35、D-37、D-38 之后定）

## 2026-09-05 来自 施工统筹会话 关于 账上 commit 与会话的关联（等 gyb）
- 事项：带 skill 的 reviewer 压力场景（`plans/2026-09-05-research-loop-pressure-scenarios.md` 第六节）报：commit 只在 runs 的 launched 版上，runs 行把 commit 和发射它的 run 会话连着、没有字段连到写这个 commit 的会话（deploy），`common/REVIEW-CHECKLIST.md` 的 D2（rule-08 事后查谁写了什么）从账上定不了。读 commit 的口子由 D-37 的 `--history` 解决，关联字段要不要加统筹不代裁。
- 裁决原文：（待 gyb）
- 要改的地方：
  51. 若加关联：`03` runs 行或 sessions 行记写这个 commit 的会话（比如 runs 的 launched 版加 `commit_session_id`，或者 sessions 行记 commit 区间），`09` 的 D2 照查账写；若不加：`09` 的 D2 改成「查 git 提交作者时间与 sessions 账的活跃区间」。代码里 D2 标 `PENDING(issue 51)`。
- 状态：等 gyb
...
- 要改的地方：
  49. 冻结后待议：（a）`03` 九本账公共骨架加可选栏 `agent_id`（子会话写的行填）；`03` sessions 字段表加 `agent_id` 一栏，`launched_by` 取值含 subagent，补一句「同一 `session_id` 下 `agent_id` 不同的行是不同的版本链，『closed 会话再写拒收』按（`session_id`，`agent_id` 或空）配对查」；（b）`04` 第四节「subagent 加载角色 skill 那一刻和普通 session 一样登记进 sessions 账」那句改成 D-15 的（3）；`04` 第六节销号钩子补 SubagentStop 按 `agent_id` 关行并交回该子会话开干的单、SessionEnd 交回本 `session_id` 名下全部（含被杀的子会话的）再关子会话行最后落母会话 closed 版；`04` 第七节 `rl session` 一族对应改；`04:21` holder 那句补「holder 仍是 `session_id`，子会话接的单 start 版带骨架可选栏 `agent_id`，查 holder 与销号找单按（`session_id`，`agent_id` 或空）配对」；（c）`05:13` 到 `05:25` actor 判定那段改成 D-15 的（1）：先看钩子注入的 `RL_AGENT_TYPE`、`RL_AGENT_ID`，再看状态文件，都没有是裸终端 gyb；不认识的类型拒写退出码 3；（d）`04` 会话生命周期一节加一句「交互会话 `/exit` 选『挪到后台』等于旧会话销号（旧 id 的 SessionEnd 已交回名下的单子）、新 id 是裸会话，要继续干活得重新加载角色」（D-20，实测在 `plans/2026-09-05-research-loop-verify.md` 3.4 节）；（e）`04` 第四节派活那段加「用打印模式或 workflow 起的派活会话要带 `CLAUDE_CODE_PRINT_BG_WAIT_CEILING_MS=0`，否则母会话默认只等后台子会话 600 秒、到点终止且子会话和母会话都不触发销号钩子」，`04:129`「钩子漏掉的会话有两道兜底」旁注这一种漏法（D-23，实测在 verify.md 3.7 节）。
- 状态：等最后一期（代裁 2026-09-05，gyb 审 D-15、D-20、D-23 之后定）
