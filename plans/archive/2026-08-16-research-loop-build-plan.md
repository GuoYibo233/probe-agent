...
issues：`id` 形如 `iss-0031`；`assignee` 五个角色或 `gyb`；`status` 取 `open`、`answered`、`closed`；`kind` 九选一；`stage`（`failed` 时必填）；`text`；`reply`（`answered` 时必填）；`handoff_id`（`cannot`、`failed`、`denied`、`withdrawn`、`orphaned` 时必填，其余可选）；`log_tail` 可选。改派等于追加一版换 `assignee`；`assignee` 是 `gyb` 的那一版（含首次开单）触发桌面通知。`reply` 只有 `assignee` 或 gyb 能写；`close` 只有开单的 actor 或 gyb 能写，通知类 issue（`withdrawn`、`orphaned`、`fyi`）的 `assignee` 也能关。另有一处自动关：`rl handoff accept` 关这张单关联的 `answered` issue。

handoffs：`id` 形如 `ho-0012`；`work_type` 三选一；`from_role`（就是 owner，可以是 `gyb`）、`to_role`；`holder`、`last_holder`；`status` 七选一；`parent_id`（`launch_order` 必填指工单，`analysis_order` 可选，快车道补单空）；`supersedes` 可选；`batch` 可选，调用者传的自由文本、rl 不分配，`launch_order` 开单时从父单抄；`line` 由 rl 从 `decision_refs` 第一项的 `root_id` 算出来存着；`dispatch` 三选一；`quick_lane` 布尔；`ql_tag`（快车道补单必填，指它合回的那条 scratch 行；其余单子空）；`adopted`（布尔，`start` 那一版可选，认领已发射未收尾的 run 时为 true）；`decision_refs` 列表，每项 `{"id":...,"version":...}`（`work_order` 必填至少一项，`launch_order` 开单时从父单抄）；`evaluation_refs`（`analysis_order` 用，每项 `{"id":...,"version":...}`，开单时可以是 `proposed`，进 `done_pending_review` 时每项必须 `approved`）；`explanation`（`work_order` 必填；快车道补单由 deploy 写并含 gyb 点名原话）；`report_paths` 形如 `{"method":...,"detail":...}`（`work_order` 进 `done_pending_review` 时 `method` 必填且存在，`detail` 非快车道时必填且存在）；`code_paths` 列表（`work_order` 进 `done_pending_review` 时必填）；`output_paths` 形如 `{"notebook":...,"figures":[...]}`（`analysis_order` 进 `done_pending_review` 时必填且存在）；`attempts` 列表只在 `launch_order` 上，每项 `{"attempt":序号,"command","args","workdir","track","config":{...},"run_id","estimated_seconds","actual_seconds","step_table":[...]}`，开单时第一项必填 `command`、`workdir`、`track`、`config`（字典：`model`、`params`、`dataset`、`split`、其余超参自由），`run_id` 由 rl 按 `<ho-id>-a<attempt>` 分配；`step_table` 每项 `{"step","kind":"gpu"|"cpu","smoke_seconds","scale_factor","estimated_seconds"}`，`estimated_seconds` 只加总最新一次尝试的行；`progress_note`（只在 `in_progress` → `todo` 那一版必填；`rejected` → `todo` 不要求）；`reason`（`rejected`、`withdrawn` 时必填，角色会话发起的 `withdrawn` 还要 `quote`）；`issue_id`（`stuck` 时必填）。

runs：`run_id`（主键，形如 `ho-0013-a1`，和产物目录名、tmux session、commit message 一致）；`handoff_id`、`attempt`；`status` 取 `launched`、`finished`、`adopted`（`adopted` 版是 run 会话认领已发射没收尾的那次尝试时由 `rl handoff start` 顺带写的，记新 holder 的 `session_id` 和 `ts`，不另设 `rl run adopt`）；`launched` 版必填 `commit`、`command`、`host`、`gpus`、`log_path`、`tmux_session`、`watch_cmd`、`started_at`、`config`（从发射单抄），产物目录按约定是 `<artifact_root>/<run_id>/`、账上不记；`finished` 版必填 `finished_at`、`exit_status`（`ok`、`failed`、`killed`）、`actual_seconds`（rl 从两个时间戳算；handoffs 发射单最新一次尝试上的同名栏由 rl 在 `run finish` 时从这里抄、人不填），`exit_status` 是 `ok` 时 `metrics`（键是指标名、值是数）和 `data_path`（产物目录里给 analysis 算数用的那一个文件或子目录）必填。`actor` 是 `run` 或 `gyb`。

grants：`id` 形如 `grant-0003`；`grantee` 角色；`permission` 字符串，第一版只有一种 `read:notes`；`status` 取 `active`、`revoked`；`expires_at` 可选；`text`；`issue_id` 可选。授权只有 gyb 能写：`actor` 必须是 `gyb`。裸终端直接写；角色会话里 `--as-gyb --quote` 替 gyb 写也收，`session_id` 照记那个会话。

feedback：`id`；`target`（文件路径，或带前缀的编号 `rule-06`、`principle-06`）；`text`；`status` 取 `proposed`、`accepted`、`rejected`；`verdict_text`；`applied_to`（采纳时必填，路径列表，rl 校验每个路径存在，至少写一个、不要求母版和文档两样都有）；`rules_version_after`（采纳时 rl 自动填）。提的那一版谁都能写，裁的那一版 `actor` 必须是 `gyb`。
...
scratch：主键是 `ql_tag`；`status` 取 `open`、`merged`、`dropped`；`role`（`deploy` 或 `analysis`）；`open` 版必填 `worktree`（deploy）或 `dir`（analysis）、`base_commit`、`branch`；中间版自由（建议 `note`、`metrics`）；`merged` 版必填 `handoff_id`（写序是先开补单拿到编号再 `rl ql close --merged --handoff ID`，补单那头的 `ql_tag` 栏反过来指这条 scratch 行，两边互指；`merged` 只对 deploy 的快车道开，analysis 的快车道只有 `dropped`）；`dropped` 版必填 `reason`。`actor` 是 `deploy` 或 `analysis`。
...
| `todo` / `stuck` | `todo`（内容追加） | owner、`to_role` | 只改内容：`launch_order` 追加一次尝试；`work_order` 补 `report_paths` 或 `code_paths`；`analysis_order` 补 `evaluation_refs`；换 `decision_refs` / `evaluation_refs` 里的引用（doctor 悬空引用的修法）；状态不变 | 无 | `handoff amend` |
...
`accepted` 和 `withdrawn` 是终态。入账脚本只认这张表，表外的转移一律拒收，退出码 2；gyb 同样，`--force` 越不过表外转移，硬改状态走 `withdraw` 再重开。`stuck` 的单子销号和 reclaim 都不动（holder 已空），只有 issue `answered` 之后 `resume` 才回 `todo`。
...
`rl inbox` 谁需要谁敲，不是上线动作：角色被拉起不自动查收件箱，先干拉它起来的那张单。查询命令是 show、list、trace、status、inbox（只读，不顺带关任何 issue）、stale、doctor 七类，谁都能调，不进 `ledger_writes`；`rl doctor --ack` / `--unack` 是写命令、只有 gyb 能敲。
...
| `rl run add --handoff ID --attempt N --commit ... --host ... --gpus ... --log ... --tmux ... --watch-cmd ...` / `rl run finish RUN_ID --exit ok\|failed\|killed [--metric k=v ...] [--data-path P]` / `rl run relink RUN_ID --handoff ID` / `rl run show RUN_ID` / `rl run list [--handoff ID] [--decision ID] [--batch B] [--line L] [--all]` | 数字账三版（`launched`、`finished`，另有 `adopted` 由 `rl handoff start` 认领时顺带写），`add` 从发射单抄 command、config、run_id；`finish` 算 actual_seconds、同进程跑反常预警、调宿主收尾命令模板；`list` 默认只出每张单最新尝试且 `exit_status=ok` 的行，`--all` 全出；`relink` 是 doctor 修法，`status` 与其他栏照抄最新版、只换 `handoff_id` | 写 run 和 gyb，查谁都行 |
| `rl grant add --to R --permission P --text ... [--expires ...] [--issue ID]` / `rl grant revoke ID` / `rl grant list` / `rl grant show ID` | 授权；裸终端直接写，角色会话里 `--as-gyb --quote` 替 gyb 写也收 | 写 gyb，查谁都行 |
...
| `rl reclaim [--session-older-than H] [--handoff-older-than H] [--only ID ...] [--skip ID ...] [--kill] [--apply]` | 列出超过阈值没动的会话和单子和快车道，`--apply` 才动手：会话标 `reclaim` 销号并 release 名下开干的单；开干的发射单默认不杀进程（留给认领），`--kill` 才走中断收尾（杀进程、释放显存、宿主销号、runs 落 killed）；`stuck` 的只把 issue 改派 owner；被打回的单子超过 `reclaim.handoff_idle_hours` 没动的推回 `todo`（`actor` 记 gyb、`via=reclaim`），owner 重新拉起；`done_pending_review` 和 `todo` 的只列出附现成命令；结束按 owner 分组打印待拉起的单子和加载命令，并自动跑一遍 doctor | gyb |
| `rl doctor [--ack ITEM ID] [--unack ITEM ID] [--list-acks]` | 扫九本账，每项附修法命令和「修完归谁推」：编号重复；引用悬空；`stuck` 单子没有 issue（修法 `issue link`）；`cannot`/`failed`/`denied` 且 open 且没有单子指回的 issue；`done_pending_review` 的交付物路径不存在（修法 `handoff amend`）；runs 行 `handoff_id` 为空、悬空或不是 launch_order（修法 `run relink` 或 `--ack`）；runs 有 `launched` 版长期没 `finished` 版；runs 挂在 `withdrawn` 的单子上；快车道工单已 accepted 但没有关联发射单；approved 口径引的 metrics 键在 runs 账里不存在；`answered` 超过 N 天没 close 的 issue；单子回了 `todo` 而关联 issue 还 open；`in_progress` 的 holder 不在活着的会话里；sessions 没有结束版且超阈值没写账；retired 决定名下有活单；决定来源指 notes/ 但 grants 里没有该 actor 的 read:notes；accepted 的 feedback 的 `applied_to` 为空；loop/runs.jsonl 与宿主 ops/runs.jsonl 对不上的 run_id；sessions 行 `model` 是 `unknown`（第 19 项，修法 `session amend`；2026-08-17 从待验证第 10 条升上来） | 查谁都行，角色跑只看不修，修法报给 owner 或 gyb；`--ack` / `--unack` 只有 gyb |
| `rl notify --text` | 内部用，发桌面通知，机制见待验证第 6 条；推送表：issue assignee 变 gyb（含首次开单）、单子进 `done_pending_review` 且 owner 是 gyb 或 `dispatch=manual`、feedback 提出、owner 无活会话的 todo 出现（在单子落 todo 那刻和 `session end` 销号时查并推）、doctor 有没修的；gyb 也可手动调给自己发提醒，两种调法都不进账 | rl 自己，gyb 手动也行 |

退出码六个：0 成功；1 rl 内部错误；2 校验拒收（原因写到标准错误，包含下一步该做什么）；3 角色无权（含角色带 `--force`、被销号会话再写账，`--as-gyb --force` 也越不过；原因写到标准错误，附「开 issue 给谁」的命令）；4 文件锁等待超时（等 `lock.timeout_seconds`，默认 10 秒）；5 用法错（参数写错、编号不存在，附正确用法或查不到的编号）。所有非零退出，标准错误第一行是固定格式的原因种类（2 `validation`、3 `forbidden`、4 `lock_timeout`、5 `usage`、1 `internal`），`--json` 时同一个值放 `error.kind`。所有子命令支持 `--json`；`rl status --json` 每行至少含 `id`、`work_type`、`owner`、`holder`、`holder_alive`、`status`、`age_hours`、`line`、`decision_refs`、`batch`、`log_path`、`watch_cmd`。

锁：`loop/.lock` 一把全局文件锁，扫号、分配编号（含 `ql_tag`、`run_id`；`batch` 是调用者自由文本，rl 不分配）、追加三步在同一把锁里；跨账写入按设计文档「账本」一节的写序，先 issue 后 handoff。
...
| Phase 5 采样器接管 | 看门狗 monitor 只在 run 上线时起，只判、只写自己的状态文件，卡死和超时分开判，每轮顺带查本单是不是 `withdrawn` 或被 reclaim；run 会话每轮读状态文件，杀进程和写账都由 run 会话做；看门狗只许调 rl 查询命令、不留痕 | 卡死阈值沿用 new1 `ops/verdicts.py` 的自适应判定线，超时用最新尝试的 `estimated_seconds` 乘宽松系数；monitor 不写九本账 |
...
| `lock.timeout_seconds` | 10 | 文件锁等多久算超时，超时退出码 4 |
...
| 4 | skill 头部禁止模型调用的声明能不能锁入口 skill | 加声明后让模型自己调一次 | 调不动 | 入口 skill 的 SKILL.md 第一行写「模型调用即违规」靠纪律，另外 rl init 检查调用者状态文件不是任何角色（2026-08-17 裁：这条备案升正案，`rl init` 只在裸终端跑） |
...
| 10 | 钩子输入里有没有模型标识 | 打印 SessionStart 钩子的输入 JSON | 有 model 字段 | sessions.model 记 `unknown`，doctor 列出来，gyb 事后补（2026-08-17 裁：已收成 doctor 第 19 项，修法 `rl session amend ID --model M`） |
...
9. actor 与权限：analysis 调 `rl grant add` 退出码 3；analysis 会话里 `rl eval approve --as-gyb` 缺 `--quote` 退出码 2、带 quote 通过且账行 actor 是 gyb、session_id 是当前会话；裸终端 `rl eval approve` 直接通过、session_id 是 `cli`；裸终端 `rl grant add` 通过、角色会话 `--as-gyb --quote` 的 `grant add` 也通过且账行 actor 是 gyb、session_id 是当前会话；非 run 调 `rl run add` 退出码 3、gyb 调通过；gyb 缺必填字段拒收、带 `--force --reason` 通过且账行有 force_reason。
10. runs 账 schema：发射版缺 `commit` 或 `config` 拒收、不要求 metrics；收尾版 `exit_status=ok` 缺 `metrics` 拒收；`failed` 不要求 metrics 但要有 actual_seconds；`finish` 时指标落在 0 或 1 自动开 `anomaly` issue 给 gyb；`run list` 默认只出每张单最新一次尝试且 `exit_status=ok` 的行、最新一次不是 ok 的那张单不出，`--all` 全出。
...
15. 收回与接替：对 `in_progress` 的单子 withdraw，自动出现 `withdrawn` 通知给 holder 的角色和 owner，从别的状态收回不通知；`--cascade` 沿 parent_id 收掉派生的发射单；`reissue` 收旧开新、supersedes 指旧单。
16. 快车道：`ql open` 分标签建 worktree、scratch 开张行；`ql close --merged` 只对 deploy 的快车道开、要求 handoff 是它的快车道补单且两边 `ql_tag` 与 `handoff_id` 互指，analysis 的快车道只有 `--dropped`、要 reason；status 只列没关的。
...
| 5 公共母版 | `common/GLOBAL-RULES.md`（第十三节八条加十一条原则，带 rules_version 和 rule-NN/principle-NN 编号）、`common/GLOSSARY.md`（第二节词表加「它不是什么」）、`common/SPEC-TEMPLATE.md`（五栏）、`common/READING.md`（读法栏原话）、判断类检查的问题清单（文件名待定，reviewer 派 sonnet subagent 按它逐题查，查出来的写进 review/ 清单、不开 issue，要不要开 issue 由 gyb 看完用自己的权限开，2026-08-17 裁） | gyb 逐条过 | 步 3 | 主会话写，中文底稿给 gyb 过，正式版英文 |
...
总验收的五个任务在步 8 前定；第一节末尾（a）到（i）九条我改的设计变动逐条裁——2026-08-17 已裁（sync-inbox 问题 27）：（b）（c）认，（d）不认，grants 可以在角色会话里 `--as-gyb --quote` 替 gyb 写；第二节到第八节的草案逐条过，过的时候一次一节；第三轮模拟要不要跑、跑之前先把 new1 第一次 rl init 那个没跑成的场景补上；new1 CLAUDE.md 的两处宿主改动（GPU 铁律对 run 会话的读法、脏树白名单加 loop/）在步 7 时由 gyb 亲手改。
...
- 2026-08-17，按 `05-rl-cli.md` 定稿的裁决（gyb 原话见设计文档同日那条）：第四节表外转移句补 gyb 同样、`--force` 越不过；第六节 `rl notify` 行推送表第 4 条补时机、gyb 可手动调不进账；`rl decision stale` 签名改 `[--handoff ID] [--all]`；`rl handoff done` 去 `--actual-seconds`；第三节骨架加 `via`；转移表 release 行补 `via`；第八节阈值表加 `lock.timeout_seconds`；`rl init` 行补只在裸终端；待验证第 4 条备案升正案、第 10 条备案收成 doctor 第 19 项；施工步骤公共母版行加判断类问题清单（查出的写清单不开 issue）；Phase 5 看门狗补只查询不留痕。对回原则 1、4、6、8、10、11。
- 2026-08-17，按 sync-inbox 问题 6 的裁决（定义处 `14-role-reviewer.md`；gyb 原话「全给我审查，然后我用我的权限放到issue里面」），第十一节步 5 的判断类检查问题清单那格补一句：查出来的写进 review/ 清单、不开 issue，要不要开 issue 由 gyb 看完用自己的权限开；上面那条 `05` 定稿回写行同补。对回原则 2、原则 5。
- 2026-08-17，按 sync-inbox 问题 7 的裁决（定义处 `04-handoffs-and-sessions.md`；gyb 原话「冒烟也是正式动作」），第四节转移表加一行 `in_progress` → `in_progress`（内容追加），holder（run）写，只改最新一次尝试的 `step_table` 与 `estimated_seconds`，子命令 `handoff estimate`。对回原则 4。
- 2026-08-17，按 sync-inbox 问题 8 的裁决（定义处 `04-handoffs-and-sessions.md`；gyb 原话「A」），第四节转移表快车道补单行的前提改成「`ql_tag` 指的 scratch 行状态是 `open`」，第三节 handoffs 加 `ql_tag` 栏、scratch 那段补写序与两边互指，第六节 `handoff open` 与 `ql close` 两行、测试 16 同改。对回原则 7、原则 9。
- 2026-08-17，按 sync-inbox 问题 9 的裁决（定义处 `03-ledgers.md`；gyb 原话「A」），第六节锁那句里的分配编号去掉 `batch`、写明 `batch` 是调用者自由文本 rl 不分配，第二节词表和第三节 handoffs 的 `batch` 同改。对回原则 9。
- 2026-08-17，按 sync-inbox 问题 10 的裁决（定义处 `04-handoffs-and-sessions.md`；gyb 原话「A」），第四节转移表两条 amend 行放宽成允许换 `decision_refs` / `evaluation_refs`、`done_pending_review` 上也允许改 `code_paths`，第六节 `handoff amend` 签名加 `[--decision ID@V ...]`。对回原则 4。
- 2026-08-17，按 sync-inbox 问题 12 的裁决（定义处 `03-ledgers.md`；gyb 原话「B」），测试 10 的 `run list` 那句改成「默认只出每张单最新一次尝试且 `exit_status=ok` 的行、最新一次不是 ok 的那张单不出」。对回原则 10。
- 2026-08-17，按 sync-inbox 问题 13 的裁决（定义处 `04-handoffs-and-sessions.md`；gyb 原话「B」），第三节 `attempts` 每项加 `actual_seconds`，runs 的 `actual_seconds` 那格补「handoffs 发射单最新一次尝试上的同名栏由 rl 在 `run finish` 时从这里抄、人不填」。对回原则 10。
- 2026-08-17，按 sync-inbox 问题 14 的裁决（定义处 `03-ledgers.md`；gyb 原话「A」），第六节退出码 3 那条补「含角色带 `--force`、被销号会话再写账，`--as-gyb --force` 也越不过」。对回原则 1。
- 2026-08-17，按 sync-inbox 问题 15 的裁决（定义处 `03-ledgers.md`；gyb 原话「B」），第三节骨架的可选栏删掉 `fix_for`、只剩 `force_reason` 和 `via`，第二节词表删掉「修账标记 `fix_for`」。对回原则 4。
- 2026-08-17，按 sync-inbox 问题 16 的裁决（定义处 `07-quick-lane.md`；gyb 原话「A」），第三节 scratch 与第六节 `ql close` 行写明 `merged` 只对 deploy 的快车道开、analysis 的快车道只有 `--dropped`，测试 16 同改。对回原则 7。
- 2026-08-17，按 sync-inbox 问题 17 的裁决（定义处 `04-handoffs-and-sessions.md` 与 `03-ledgers.md`；gyb 原话「C」），第三节 handoffs 加 `adopted` 栏、runs 的 `status` 加 `adopted` 版，第四节转移表 start 行与第六节 `handoff start`、`run` 两行同改。对回原则 11、原则 9。
- 2026-08-17，按 sync-inbox 问题 19 的裁决（定义处 `04-handoffs-and-sessions.md`；gyb 原话「B」），第六节 `rl reclaim` 那行加第五类处置：被打回的单子超过 `reclaim.handoff_idle_hours` 没动的推回 `todo`（`actor` 记 gyb、`via=reclaim`）。对回原则 3。
- 2026-08-17，按 sync-inbox 问题 20 的裁决（定义处 `04-handoffs-and-sessions.md`；gyb 原话「B」），第四节转移表 withdraw 行改成「从 `in_progress` 收回时通知 holder 的角色和 owner，其他状态不通知」，测试 15 同改。对回原则 6。
- 2026-08-17，按 sync-inbox 问题 21 的裁决（定义处 `04-handoffs-and-sessions.md`；gyb 原话「A」），第四节转移表 reissue 行的「到」栏由「同状态（接替）」改成「旧单 `withdrawn`，新单 `todo`（`supersedes` 指旧单）」。对回原则 3。
- 2026-08-17，按 sync-inbox 问题 22 的裁决（定义处 `03-ledgers.md`；gyb 原话「A」），第三节 runs 的 `run_id` 那格删掉「快车道用 `ql_tag`」，快车道数字只进杂账。对回原则 7。
- 2026-08-17，按 sync-inbox 问题 23 的裁决（定义处 `05-rl-cli.md` 与 `03-ledgers.md`；gyb 原话「只有做完了的时候才关，巡检要我本人确认」），第三节 issues 的自动关两处改成一处（只留 `handoff accept`）、close 写权补「通知类 issue 的 assignee 也能关」，第五节查询命令那句和第六节 `rl inbox` 行写明只读不关，`rl doctor --ack` / `--unack` 列为写命令、只有 gyb。对回原则 6、原则 2。
- 2026-08-17，按 sync-inbox 问题 25 的裁决（定义处 `03-ledgers.md`；gyb 原话「我想让agent有办法识别发生了什么就行」），第六节退出码由四个改成六个（加 1 内部错误、5 用法错），补「所有非零退出标准错误第一行是固定格式的原因种类，`--json` 时放 `error.kind`」。对回原则 6。
- 2026-08-17，按 sync-inbox 问题 26 的裁决（定义处 `03-ledgers.md`；gyb 原话「我觉得。有一些改的方法，不一定会改公共规矩，如果是这样的话就选b。」），第三节 feedback 的 `applied_to` 补「至少写一个、不要求母版和文档两样都有」，第六节 doctor 第 17 项改成「`applied_to` 为空」才报。对回原则 4。
- 2026-08-17，按 sync-inbox 问题 27 的裁决（定义处 `01-gyb.md`；gyb 原话「3 不是，可以替我写」），第一节（b）（c）标已裁、（d）改成「授权只有 gyb 能写，裸终端直接写，角色会话里 `--as-gyb --quote` 替 gyb 写也收」，第三节 grants、第六节 actor 判定段与 grant 行、测试 9、第十二节留给 gyb 那条同改。对回原则 1。
- 2026-08-17，按 sync-inbox 问题 28 的裁决（定义处 `01-gyb.md`；gyb 原话「每个角色创建时候，不要自动查收件箱」「C」「顺便run只需要关注自己的工单，一般不会空run，不需要查，这个改了」），第五节「所有角色上线第一个动作：`rl inbox`」和第六节 `rl inbox` 行的「上线第一动作」都改成「谁需要谁敲，不是上线动作」，第五节 run 那句「run 的 `rl inbox` 不查过版」扩大成「run 不查 inbox」。对回原则 6。
- 2026-08-17，按 sync-inbox 问题 31 的裁决（定义处 `03-ledgers.md`；gyb 原话「a」），第六节 `rl run relink` 那句补「`status` 与其他栏照抄最新版、只换 `handoff_id`」。对回原则 4。
- 2026-08-17，按 `05-rl-cli.md` 定稿（冻结 `77213e5`）的命令表对齐（rl-hub-v3 审后补，05 定稿那一轮漏传的四处）：第六节 `handoff release` 签名改成 `release ID [--note ...]`（`in_progress`→`todo` 时必给，`rejected`→`todo` 不要求）；`rl session` 行加 `session amend ID --model M` 及只许改 `model`、closed 也能 amend、谁能调；`rl grant add` 加 `--text ...`，`rl feedback accept` 的 `--text` 改成必给；`rl doctor` 行补第 19 项 sessions `model` 是 `unknown`。
