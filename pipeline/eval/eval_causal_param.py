...
def main():
    ...
    # ctool 剔除的行不许当触发点候选(spec 16.2 衔接段):提前读 excluded_idx,
    # 只在剩下的行里挑触发点——零 logits 过 softmax 是均匀分布,只在
    # θ<=1/n_labels 时才会被 θ 天然挡住,不能靠这个当保险。一个事件的候选行
    # 全部被剔时没有触发点,不判分,计 n_excluded_by_ctool。
    ...
    # 三步顺序写死:readonly 排除 -> --overlong 筛选 -> --limit)。ctool 剔除
    # 已经在挑触发点那一步处理过(上面的 n_excluded_by_ctool),这里传空集合,
    # 只做提示长度筛选——keys 里的事件都已经保证至少有一个未被 ctool 剔除的
    # 候选行,select_keys 的剔除分支在这里必然不再命中。
