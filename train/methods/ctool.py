...
# TODO(gyb, 2026-09-22): the owner does not understand the packing below yet and comes back to
# read it; nothing here is judged wrong and nothing is to be changed on this note. The parts to
# read: `_build_events` (the shared prefix and each row's own tail, from the longest common
# token prefix), `_chunk_by_budget`, and `_make_batch` (the position ids, the block mask of who
# may attend to whom, and the event_end position each row's logits are read at). Worth knowing
# while reading: the alignment gate in train/utils/trainer.py compares the packed loss with the
# plain loss on the first events_per_mb * accum rows only, so it proves those rows and no
# others. train/methods/cgen.py and train/methods/cparam.py carry their own packing.
