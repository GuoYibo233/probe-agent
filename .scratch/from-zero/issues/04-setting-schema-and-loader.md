...
- 2026-09-18, owner rulings applied at the wave-4 closeout (main session of
  ...
  report `T04-schema-ruling-report.md`; branch
  `ticket/2026-09-18-wave4/T04-schema-ruling`, `ba97dd9`, merged as `2169c4e`).
  `train.warmup_ratio` defaults to `0.05`; `inject.chunk_tokens` and
  `inject.tail_tokens` are removed from the `Inject` dataclass and from
  `STAGES["inject"]["sections"]`, so a YAML that still writes one is refused by
  the unknown-field check. `train.checkpoint_hours` stays in the key (ruled).
  This ticket's own text is now stale in the places the re-reviewer listed:
  the `Inject` defaults table, the pasted `STAGES['inject']['sections']` tuple,
  the legacy provenance line, `B2`'s `i.chunk_tokens` / `i.tail_tokens`
  dereference, and the `warmup_ratio` default; ticket 11's C4/C5 fixtures pass
  the two names into a `SimpleNamespace`, which still runs.
