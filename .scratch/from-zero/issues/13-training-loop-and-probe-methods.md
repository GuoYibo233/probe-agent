...
- 2026-09-18, wave 5 reconciliation (main session of wave 5, session new1-97). Resolved. Commits `f520b87..e128671` on `ticket/2026-09-18-wave5/T13` (implementation `eaa105d..1e4645f`, fix round 1 `27485a0`, fix round 2 `e128671`), merged as the first merge after `f520b87`; ticket 14's `README.md` was taken wholesale at the second merge and this ticket's four blocks discarded (the two copies differ in layout, not in the imports, used-by, reads, writes or venv they state). Two fix rounds: round 1 added the VERSION rule block and `VERSION_HISTORY = {}` to the three method files, made epoch-end validation fire when a method drops a train event, gave each epoch its own shuffle (`trainer.run` hands `batches` a copy of `cfg` whose `train.seed` is `seed + epoch`), and made cparam's `predict` write one row per example row; round 2 kept `device=` on the two `base.load` calls, which ticket 06's pinned signature carries and 2.6's does not. On the merged tree A3.1, A3.2, A3.6 (3 tests OK) and A3.7 pass, and every stage of `train_probe.yaml` and `inject.yaml` now keys (`run.py where` prints `train`, `eval` and `inject` directories). Minors left open: cparam's `predict` writes `target = None` for a row whose call does not assemble as `tool + "("`, and `eval/utils/probe_eval.py`'s cparam branch concatenates `target` unguarded (F6); a comment at `trainer.py:167` attributes a signature to contracts 2.6 that only 6.2 carries (N1). Shelved, nobody depends on them yet, recorded for the owner: (a) a resume from `last/` does not carry the pre-crash best objective, so the first post-resume validation becomes `best/` even when worse (GPU item 3 exercises it); (b) `counts.dropped_overlong` counts the train split only, drops that happen on val/test rows in `predict` are not counted; (c) the ticket's alias names `trainer.example` and `trainer.prediction` are stale, the modules are `trainer.training_data` and `trainer.probe_output`. The seven GPU items are BLOCKED for the main session; `run.py` now exists, so their commands are runnable as written.
- 2026-09-19, wave 5 post-merge review and fixes (session new1-97; records `post-merge-review.json`, `post-merge-fix.json`, `post-merge-fix-train.md`, `post-merge-fix-probe.md` in the wave directory). Verified on this ticket's files and fixed on `fix/2026-09-18-wave5-train` (three commits ending `225d951`, one opus fix round, re-review clean) and `fix/2026-09-18-wave5-probe` (`5298502`): the epoch-end flag fired at every logical minibatch, so `train.accum` never accumulated, the run stopped after `1/accum` of the data and validation ran per minibatch (critical, a regression of fix round 1); every method built its labels, weights, `target_ids` and `row_of` on the CPU while the logits sit on the card, so the first loss on a GPU raised (critical); training mode was never set on a resume or with `align_check: false` (important); `reference_loss` scored rows `batches` drops and truncated where `batches` does not, so the gate failed on correct packing (important; the pinned "left-truncated prompt" is dropped, errata entry of the wave-5 post-merge review); a `--debug` train (`max_steps: 20`) wrote no step line under `LOG_EVERY = 50`, so the first and the last step now log too; the prediction step emitted no beat and a predict-only continuation none at all (unit `step`, `total = len(splits) + 1` now); the warmup comment stated `0.0` where the schema has `0.05`. In `models/probe_models/base.py`: a restore built no LoRA adapter, so a LoRA run could not resume, and a restore on a card came back in the serving `bfloat16` (`VERSION = 2`, stale `train`). Minors left open (fix reports): the stall line over a long prediction pass and `ls` progress jumping to `0/3` when prediction starts; a gate slice with every event over `max_len` passes vacuously; a resumed LoRA run's Adam moments meet a fresh adapter. On the merged tree A3.1, A3.2, A3.6 (three tests, six subtests) and A3.7 pass.
