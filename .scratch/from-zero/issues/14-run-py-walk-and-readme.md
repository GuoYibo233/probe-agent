...
- 2026-09-18, from gyb (errata "3.3 / 8.6 (what a `VERSION` bump invalidates)"): `run.py ls`'s `behind` flag tells two cases apart. A run whose recorded `_versions` are lower than the files' current `VERSION` **and** whose key still equals the key its setting computes today is `behind` and usable (the bumps between did not make its stage stale, per the files' `VERSION_HISTORY`); a run whose key no longer equals it is stale, and `ls` says which file's bump, quoting that entry's `why`. `run.py` reads the tables through `experimental_settings/schema.py`, never by parsing a file itself.
- 2026-09-18, wave 5 precheck (main session of wave 5, session new1-97; record in `.scratch/from-zero/sdd/2026-09-18-wave5/precheck.json`). Three facts for the README rewrite and the acceptance, no change to the body: (1) contracts 0.2 predates three owner rulings applied on this branch, so section 2 of `README.md` names the files as they are on disk and in the last four errata entries: the `eval/methods/` folder is folded into `eval/utils/probe_eval.py` (errata "0.2 / 2.1 / 2.6", merged f13f0ad), the four `agent/` files are `run_tasks.py`, `step_without_probe.py`, `step_with_probe.py`, `injected_text_formats.py` (errata "0.2 (the four `agent/` file names)", commit dabb56e), and the tree has **31** Python files, not 34; `D1` still expects `31 True`. (2) `experimental_settings/schema.py` now offers `version_history(rel_path)` and `effective_version(rel_path, stage)` (errata "3.3 / 8.6", merged 39b3d60); `ls`'s `behind` flag reads them. (3) The read-only hook refuses any Bash command line that names `experimental_settings` or a settings yaml together with `cp `, `tee`, `>` or another write word, whatever the command does. `C5`'s `cp -a run.py constants experimental_settings ...` is therefore refused as written: build the private copy from the branch's last commit instead, `git archive HEAD run.py constants experimental_settings data models agent eval jobs | tar -x -C "$T5"` (commit first; the rest of `C5`, which edits only `$T5/constants/path_outputs.yaml`, stays as written).
- 2026-09-18, wave 5 reconciliation (main session of wave 5, session new1-97). Resolved. Commits `f520b87..6e85a88` on `ticket/2026-09-18-wave5/T14` (implementation `1f60634..089596a`, fix round `6e85a88`), merged as `e5a2281` with `README.md` taken wholesale. One fix round: dead `sample`/`inject` claims are released before every relaunch (F1), and `run.py` calls `jobs/launch.py`'s liveness and heartbeat readers instead of carrying copies of them, which made two of that file's private names public (`piece_alive`, `read_beats_for_run`; F2) — the one change outside the ticket's two files, kept because it removes a duplicate. On the merged tree C1 (ten names, one per line, two-space indent; the missing-workflow message exits 1), C3 (`ls`, `find`, `table` answer "no runs" without an `ssh`, `jobs/` untouched), D1 (`31 True`), D2 (31 files on disk, none missing from the README) and D3 pass. Minor left open: the README's `run.py` entry omits `heartbeat/*.jsonl` and the `ssh`/`tmux` route among its read sources (F3). Judgment calls the implementer recorded, unchanged: a `sample`/`inject` completion's `done.json` takes `versions` and `commit` from the frozen setting; the CPU-stage open-row refusal reuses `jobs.launch.gate_open_row`; `retry`, `refire` and `kill` take no `--debug`, so none of the three can target a debug run's stage (the pinned command-line list has none); the README's file headers are flush-left full paths per contracts 0.1's example, and a `CLAUDE.md` entry is included. Inherited, out of scope, for the owner: `gate_open_row`'s liveness test is tmux-based, so within `launch_timeout_s` of a CPU stage's own launch a second `run.py` could pass the open-row refusal. The card-stage launch path and the inject gates against real train and eval directories are ticket 15's D6/D7 and the construction plan's section-4 walks (GPU, main session).
- 2026-09-19, wave 5 post-merge review and fixes (session new1-97; records `post-merge-review.json`, `post-merge-fix.json`, `post-merge-fix-walk.md` in the wave directory). Verified on this ticket's files and fixed on `fix/2026-09-18-wave5-walk` (four commits ending `20aba0a`, two opus fix rounds; the second round's re-review was lost to the session limit, so its one commit, `ls` loading a setting once per stage instead of once per ledger row, is merged on the fixer's own measurement and falls to the final review): the walk never folded `done.json`'s `stage_extra` into `meta.json`, so every eval stage raised `KeyError('labels')` (critical, contracts 8.3); a sample/inject directory completing a widened request wrote no `done.json`, no teardown and no finish row (important, 2.3); `eval` and `score` were skipped on `done.json` where 2.3/2.4 say they always recompute (important; this ticket's step 2 sentence is overridden, errata entry of the wave-5 post-merge review); `ls` had no `behind` flag and printed four of 8.6's eight flags with no beat age, sessions, cards, unit or rate (important; `registry.ls` gained the parameters `run.py` passes); a pinned `key:`/`dir:` reference was resolved under the non-debug root (important; now under the root its own key names). Not a code change, recorded in the errata: the construction plan's section-4 lines 3 and 4 need `eval.theta_from` in the `key:` form. Minors left open (fix report): `_pinned_run_dir` lets `load_frozen`'s exceptions escape as a traceback; the README's `run.py` `writes:` line names only `owners` and `stage_extra` of `meta.json`; the widened `ls` row carries an `avg_rate` and a per-piece `beat_age_s` no consumer reads. On the merged tree C1, C3, D1 (`31 True`), D2 (31 files) and D3 pass, and every stage of every setting file keys.
