...
- 2026-09-17 wave 2 closeout: implementation passed review in 0 fix rounds, branch `ticket/2026-09-17-wave2/T05` (base `943af82`, head `7cc4d65`), merged as `f62eba1` (README: every ticket's lines kept). Main-session checks after the merge: both files import under the probe, appworld and vllm interpreters; `B4` over the 315 p1 trajectory files prints `4074 4040 4040`; `C0` and `C1` print the ticket's expected lines under the appworld interpreter (`spec_s` 1.534, `DIR_AFTER False`). Acceptance-script defects reported by the implementer and left in the ticket text: `A0b` loads the module by path without registering it in `sys.modules`, which raises inside `dataclasses` for any `@dataclass` under `from __future__ import annotations`; `B6`'s second block reads `.module` on an `ast.Import` node. Two implementer decisions with no acceptance coverage: `open_env` raises one combined `ValueError` naming every missing method and attribute; the bare-value scan of `build_call`. Report `sdd/2026-09-17-wave2/T05-report.md`.
- 2026-09-17 wave 2 closeout, post-merge fix rounds: the post-merge review (`sdd/2026-09-17-wave2/post-merge-review.json`) confirmed two important findings. ENV-1: the call finder behind `split_args` counted parentheses with no quote tracking, so over the p1 corpus 13 of the 4,040 parsed actions were cut inside a password string (`password="TsivP)G"` parsed as `TsivP`) and passed the round-trip gate, and the 10 actions this ticket's `B4` note calls "never closes its parentheses" were well-formed calls with a `(` inside a string; gyb confirmed the fix on 2026-09-17 although this ticket pins the quote-blind walk. ENV-2: `build_call` wrote a value with leading or trailing whitespace bare, so the re-parse lost the whitespace and 2.5's hard stop would fire. Fixed on the recreated branch `ticket/2026-09-17-wave2/T05` (base `67473dd`, head `a1e61c6`, `data/environments/appworld.py` only) in 2 rounds (the round-1 re-review raised one new important finding under its own id `ENV-3`, the writer's bare-value test did not honour the backslash escapes the new reader honours, addressed in round 2; the review's minor of the same id below is a different item): one quote-aware call walk serves `split_args` and `complete_call`, and the writer asks that same reader whether a value can be written bare or quoted. Merged as `da94bb6`. Main-session checks after the merge: `B4` now prints `4074 4050 4050`; the three finding examples (`password="TsivP)G"`, `password="b270(RP"`, `message="See you at 5 "`) parse to the full value and round-trip; `C0` and `C1` lines unchanged; imports under the three interpreters. Ticket text the fix makes outdated: `B4`'s expected `4074 4040 4040` is `4074 4050 4050`; the 34 `None` actions are 24, all code blocks that call no api, in 13 trajectories, 0.59% of the non-null actions; errata entry 2's 24 + 10 split is 24 + 0 (a new errata entry records it, and ticket 09's `counts.events_skipped_no_call` has one kind of input over p1). Decision left to the owner: a value that needs quoting and ends in an odd number of backslashes has no writing under the ticket's `.strip().strip("\"'")` parse rule, so `build_call` raises `ValueError` naming the tool, the key and the value (0 of the 5,297 p1 argument values hold a backslash); relaxing the quote-on-`=` rule for a named key would rewrite 7 real corpus calls and was measured and reverted. New minors for the final review: ENV-4 (`_stays_one_argument` refuses a bare value whose bracket depth dips below zero and returns to zero), and the review's ENV-3 (`_requote` labels an unbalanced call `unparsable_raw`), ENV-4-review (`build_call` renumbers a keyword argument literally named `posN`), ENV-5 (no `# venv:` comment line in `appworld.py`). Fix result `sdd/2026-09-17-wave2/post-merge-fix.json`.
