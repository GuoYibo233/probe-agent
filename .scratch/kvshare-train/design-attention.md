...
### 9.8 三个新配置的冒烟对第九节的校准：1.7B 每 token 3.88 MB，l4 的 CheckpointError 是重算走了 cuDNN，对齐差随底座单调涨

先摆事实（plan-8-28 2026-08-28 夜报的冒烟数，加上我从各 run 目录和日志里读的）。b17（H200，`tokens` 探针）最满块 98.25 GB、最长事件 70.42；l17（H200）77.62、49.92；b06 `cost` 三块 15.35 / 15.25 / 15.25、`loop` 两组 15.22 / 15.06（`--smoke` 只取最短的 40 个事件，块很小）。l4（H100 gpu0，`tokens` 探针，`--grad-ckpt --lora`）在探针第一次反向崩，日志 `logs/new1_ks828l4_gptoss_cgen_smoke_t108g0.log` 第 50 到 62 行原文：`torch.utils.checkpoint.CheckpointError: torch.utils.checkpoint: Recomputed values for the following tensors have different metadata than during the forward pass.`，位置 37 前向保存 `torch.Size([2, 32, 8192, 8192])` bf16、重算 `torch.Size([2, 1, 8192, 8192])`；位置 41 保存 `torch.Size([2, 32, 8192])` fp32、重算 `torch.Size([2, 32, 8192, 1])`；位置 43 和 44 保存 `torch.Size([])` int64 在 cpu、重算在 cuda。plan-8-28 的修法是 `.backward()` 也套 `_attn_ctx`（b5cee8d，训练循环与探针两处），l4 与 b06 开检查点的冒烟已重发。三个底座的 `ALIGN_CHECK.json`（同一批 6 个 val 事件 154 行 2,877 个目标 token）：b06 逐行 5.48e-6、逐 token 4.29e-5、基线 1.08e-5、`rel_max_abs_diff` 2.7e-6；b17 与 l17 9.06e-6、6.20e-5、9.54e-6、4.8e-6，两个 run 在两张不同的 H200（tokyo108 gpu3 与 gpu4）上逐位相同（9.059906005859375e-06）；l4（H100 gpu0）1.54e-5、9.82e-5、1.34e-5、8.5e-6，对门槛 2e-5 余 23%，对 16.4 的相对门槛 1e-5 余 15%。b06 的六个 GPU run（三个速度档、终验、smoke2、smoke3，H100 三张卡）逐行差全部是 5.4836273193359375e-06。

torch 2.11.0 的 `_meta_registrations.py`：`meta__scaled_dot_product_efficient_attention` 的 logsumexp 形状是 `(B, num_heads, ceil(M / 32) * 32)` fp32，`meta__scaled_dot_product_cudnn_attention` 的是 `(B, H, S_Q, 1)` fp32；efficient 的反向节点 `ScaledDotProductEfficientAttentionBackward0`（`Functions.h` 第 12364 行起）保存 `attn_bias_`、`query_`、`key_`、`value_`、`log_sumexp_`、`output_`、`philox_seed_`、`philox_offset_`；2.4 节读过的 `attention.cpp` `preprocess_mask` 把 `[B, 1, L, L]` 的掩码 `expand` 到 `[B, 头, L, L]`（视图，不复制）再交给 efficient 内核。

下面是判读。第一，plan-8-28 的判读对：位置 41 保存的 `[2, 32, 8192]` 正是 efficient 的 logsumexp 形状，重算出的 `[2, 32, 8192, 1]` 正是 cuDNN 的；位置 37 保存的 `[2, 32, 8192, 8192]` 是 efficient 反向节点保存的 `attn_bias_`，也就是 expand 之后的掩码视图，元数据是 32 个头的形状、存储仍是那一份 `2 × 8192² × 2` 字节 = 268 MB，不是 8.6 GB 的分数矩阵（真要是每层 8.6 GB，36 层 309 GB，l17 与 b17 的探针数也不会落在没有 L² 项的线性模型上），cuDNN 不 expand 所以重算出 `[2, 1, 8192, 8192]`；位置 43、44 是 philox 的 seed 与 offset，efficient 在 eager 下放 cpu、cuDNN 放 cuda。四处差异都指向同一件事：前向在 EFFICIENT 上下文里走了 efficient，反向里的检查点重算在上下文之外走了默认选择，H100 上默认就是 cuDNN（第七节第 3 步实测）。为什么之前没撞上：第七节第 7 步的 `--grad-ckpt` 验证是我的脚本，前向和反向包在同一个上下文里；训练器把上下文只包在 `_forward_packed` 里（工单 05 S3 把 `.backward()` 挪出 autocast 的时候没有一起考虑内核上下文），而 l4 的冒烟是训练器在 GPU 上第一个带 `--grad-ckpt` 的 run。修法对：`sdpa_kernel` 改的是进程级的四个开关，反向在 autograd 线程里重算也能看到，把 `.backward()` 套进去即可；CPU 测试盖不住这条（CPU 上 `_attn_ctx` 是空上下文），l4 重发的冒烟是唯一的验证。

第二，显存模型按冒烟校准。1.7B 不开检查点的每 token 系数从四个实测点反推（扣掉固定项和损失位那一项再除以 token 数）：b17 最满块 (98.25 − 30.97 − 3.57) / 16.384 = 3.89，最长事件 (70.42 − 30.97 − 2.42) / 9.504 = 3.90，l17 3.87 与 3.88，取 3.88 MB（9.1 按层内张量推的 3.68 低 5%，差额是 9.1 说过的没列进来的瞬时张量在 1.7B 上占比更大）。固定项的账也对上了：b17 与 l17 同一块的差 98.25 − 77.62 = 20.63 GB，模型里两者固定项的差是 30.97 − 10.60 = 20.37。用 3.88 重算 9.4 的表：b17 不开检查点真峰 30.97 + 63.57 + 7.69 = 102.2 GB，乘 1.1 是 112.5，H200 75%，H100 装不下；l17 不开检查点 81.9，乘 1.1 是 90.0，H100 只余 10% 不能上，H200 60%；直接用实测加损失位差额（98.25 + 1.8 × 2.287 = 102.4）得到同一个数。plan-8-28 来信里的 108.1 与 85.4 是拿 tokens 块的实测直接乘 1.1，少了 4.1 GB 的损失位差额，但两个配置的落卡结论相同。决定 30 维持：l4 开检查点三张 H100（估 42.9 乘 1.1 是 47.2，就算像 1.7B 一样高 5% 也在 50 以内），b17 不开检查点三张 H200 收完接 l17 不开检查点，b06 开检查点三张 Ada（估 23.8 乘 1.1 是 26.2，51%）；l4 与 b06 开检查点的实测等重发的冒烟，`verify/smoke_check.py` 里 1.7B 的系数已改成 3.88，4B 与开检查点的系数等实测再校。

第三，对齐差随底座单调涨，但不是随机的。三个底座逐行最大差 5.48e-6 / 9.06e-6 / 1.54e-5 是 1 / 1.65 / 2.8 倍，逐 token 4.29e-5 / 6.20e-5 / 9.82e-5 是 1 / 1.45 / 2.3 倍，基线（旧训练器单行对整批）1.08e-5 / 9.54e-6 / 1.34e-5 涨得慢。差的来源是 fp32 下两条路径的累加顺序不同（矩阵乘的 K 维分块、注意力内核），每层的舍入误差随累加长度的平方根涨、再逐层传递，4B 的层数是 0.6B 的 36 / 28 倍、`down_proj` 的累加长度是 9,728 对 3,072，按 层数 × sqrt(中间维) 算是 1 / 1.41 / 2.29 倍，和实测 1 / 1.65 / 2.8 同一个方向，实测略陡；三个点单调，我预期 8B 会再涨到 2 到 3e-5。不会因为非确定性越线：同一张卡型、同一批 6 个事件、TF32 关、efficient 内核，数值逐位可复现，证据是 b06 六个 run 三张 H100 上逐位相同、b17 与 l17 两张 H200 上逐位相同；扫描的三个 l4 run 和 l4 冒烟同在 H100 上，对齐检查在 `lora_util.wrap` 之前、不受学习率和检查点影响，三个 run 会得到和冒烟逐位相同的 1.537799835205078e-05。会动对齐差的只有换卡型（H100 换 H200 的 cuBLAS 分块可能不同）或者换 torch 版本。建议：b06、b17、l17 门槛 2e-5 不动；l4 在 GRID 的 `extra` 里显式带 `--align-tol 3e-5`，理由是 23% 的余量太薄，而 3e-5 离掩码错位那种真错的 1e-2 仍差 300 倍；16.4 的相对规则在 4B 上余量更小（8.5e-6 对 1e-5），不要拿相对规则替代。

### 9.9 开检查点的两个配置按重发的冒烟校准：l4 每 token 0.55 MB、固定项 16.3 GB，b06 每 token 0.12 MB，排卡不变，`save_merged` 副本不进排卡规则

先摆事实（2026-08-29 凌晨重发的两个冒烟，`.backward()` 套上下文的修法 b5cee8d 之后；数从两个目录的 `train_log.jsonl` 读，`verify/smoke_check.py` 的输出）。`ks828l4_gptoss_cgen_smoke2`（H100 gpu0，4B LoRA 加检查点，`tokens` 探针）：最满块（B=2、L_pad 8,192、16,384 个 token、1,984 个损失位）28.82 GB，最长事件（9,504 个 token、1,344 个损失位）23.89 GB，零 Traceback；对齐逐行 1.537799835205078e-05 和崩掉那次逐位相同；5 条 step 的 `peak_mem_gb` 是 23.885 / 18.737 / 33.089 / 33.185 / 33.18，四次评估 `val_ce` 1.5572 → 1.1088 → 0.8522 → 0.7443 每次都创新低、每次都 `save_best`，四次 `save_best` 事件距前一条 `eval` 事件 149 / 130 / 136 / 126 秒，epoch 末的生成评估 81 行 `gen_s` 35.7 秒，`wall_s` 1,003.4。`ks828b06_gptoss_cgen_smoke4`（0.6B 全参加检查点）：最满块 15.84、最长事件 13.85，step 峰值最大 13.85，`val_ce` 1.7044 → 1.2917，`gen_s` 81 行 69.4 秒，`wall_s` 663.3。

两点反推（每损失位仍按 1.8 MB）：l4 开检查点每 token (28.82 − 23.89 − 1.8 × 0.640) / 6.880 = 0.549 MB，固定项 28.82 − 0.549 × 16.384 − 3.57 = 16.25 GB；b06 开检查点每 token 0.122 MB，固定项 10.27 GB。

下面是判读。第一，开检查点的系数与固定项都改按实测。l4 的固定项 16.25 比 9.1 的 24.66 少 8.4 GB，正好是 bf16 权重副本的 8.04 GB：不开检查点的时候每个线性层为反向保存的那份 bf16 权重（l17 的实测系数和 b17 相同，说明不开检查点的时候 LoRA 一样留着这份副本）在开检查点之后只在重算的那一层里瞬时存在，所以 LoRA 加检查点的固定项是 fp32 权重加 LoRA 四份 = 16.62，和拟合的 16.25 差 0.4；全参加检查点的 b06 固定项 10.27 比不开的 10.86 少 0.6，是 autocast 缓存的 bf16 副本（1.19 GB）在反向阶段不在了的一部分。每 token 的系数 0.6B 是 0.122（9.1 按层内张量推的 0.195 里，每层 fp32 输入 4d × 28 层 = 0.115 是主项，一层重算的 0.08 在峰值时刻不在场），4B 是 0.549（推算 0.555，两项都在场）；两个底座的峰值时刻不同，所以不能用一个倍率互推，1.7B 开检查点的系数没有实测，扫描里也不用（b17、l17 都不开检查点）。9.1 里 0.6B 开检查点 0.21 MB 的推导作废：第七节第 7 步那 4.36 GB 的增量里含第一次反向分配的 2.38 GB 梯度，当时没有扣掉。

第二，扫描的真峰与排卡。开检查点下损失位那一项占大头，代价最大的块和不开检查点的不同：按 0.549 的系数 l4 最大的是 15,360 个 token 加 4,992 个损失位那一块（8.45 + 8.99 = 17.4 GB，来信里说的 4,271 损失位那一块是 9.0 + 7.7 = 16.7），真峰 16.25 + 17.4 = 33.7 GB，乘 1.1 是 37.0，H100 37%；b06 最大的是 12,736 加 5,696 那一块（1.55 + 10.25 = 11.8），真峰 22.1，乘 1.1 是 24.3，48G 卡 48%。l4 三张 H100、b06 三张 Ada 的排法不变。附带一条：按新的数 l4 开检查点在 48G 卡上是 73%，乘 1.1 之前的 33.7 加副本 16.1 是 49.8 对 51 也只差 1.2 GB，现在不动，H100 空着没有理由让 l4 去挤 48G 卡。

第三，`save_merged` 的副本不必进排卡规则，但要记进墙钟。l4 冒烟 step 3 到 5 的峰值 33.09 到 33.19 就是副本那一刻：固定项 16.25 加 fp32 底座副本 16.09 是 32.3，再加评估段的瞬时张量约 0.8；峰值出现在 `save_best` 之后的下一条 step 日志里是因为 step 的峰值计数器在写日志的时候才 reset，副本本身在 `save_merged` 里 `del merged` 加 `empty_cache` 释放了，三条 step 的数不涨（33.09 / 33.19 / 33.18）也说明没有泄漏。副本那一刻的占用和训练峰值相比：l4 32.3 对 33.7，同一量级、低 1.4 GB；l17 在 H200 上 10.60 + 6.88 = 17.5 对训练峰值 81.9；b06 与 b17 全参没有副本。所以对现在的四个配置，副本那一刻都不高于训练峰值，排卡看 `worst_gb × 1.1` 就够；规则里补一句「LoRA 配置另核 固定项 + fp32 底座大小 不超过卡容量」是为了防将来把 l4 排到小卡（上一段算的 l4 在 48G 卡上 49.8 对 51 就是要防的情形）。墙钟要补：4B 的一次 `save_merged`（深拷贝、合并、16 GB 写到 NFS）是 126 到 149 秒，`val_ce` 每次创新低就存一次，扫描里学习率线性降到 0、四个评估点大概率次次创新低，一个 l4 run 要多花约 9 分钟，9.4 估的 2 到 4 分钟偏低；生成评估按冒烟折算 200 行 b06 约 171 秒（每批 8 条 6.3 秒，比评测脚本的 3.7 秒慢七成，和 9.6 说的 fp32 权重逐步转 bf16 一致）、l4 约 88 秒（冒烟的提示是最短的 40 个事件，全长提示的预填充再加十几秒），9.6 估的 4B 3 到 5 分钟偏高，改成 1.5 到 2.5 分钟。

### 9.10 学习率扫描 12 个 run 的实测对第九节的核对：显存偏差最大 1.8%，整批 2 小时 16 分钟，12 条 val_ce 曲线无一处上升

本节只摆事实。数来自 `pipeline/runs/sweep/<run_id>/train_log.jsonl`（`verify/sweep_watch.py` 每半小时读一次，终态 2026-08-29 02:44）、plan-8-28 生成的 `pipeline/runs/sweep/SWEEP_REPORT.md` 和 `ops/runs.jsonl` 的发射记录。排卡按决定 30：l4 三个在 tokyo108 gpu0 到 2（H100，开检查点），b17 三个在 gpu3 到 5（H200，不开），b06 三个在 tokyo107 gpu0 到 2（Ada，开），l17 三个在 b17 收完之后的 gpu3 到 5（H200，不开）；预算全部 16384，探针 `cost`。

显存（GB；整程 step 峰值最大值、`cost` 探针 `worst_gb`、9.9 的真峰估计；同一配置三个学习率的两个实测数逐位相同）：b06 22.427 / 22.005 / 22.1，峰值对探针 +1.9%、对估计 +1.5%；b17 102.336 / 102.29 / 102.2，+0.05%、+0.1%；l17 81.655 / 81.654 / 81.9，+0.001%、−0.3%；l4 33.088 / 32.964 / 33.7，+0.4%、−1.8%。四个配置的整程峰值都高于 `cost` 探针的数，高出 0.001 到 0.42 GB。`tokens` 探针的冒烟数对同配置的整程峰值：b17 98.25 对 102.34（低 4.09 GB，−4.0%），l17 77.62 对 81.66（低 4.04，−4.9%），l4 28.82 对 33.09（低 4.27，−12.9%），b06 15.84 对 22.43（低 6.59，−29.4%）；不开检查点的两个差额和 9.8 说的损失位差 (4,271 − 1,984) × 1.8 MB = 4.1 GB 相同，开检查点的两个差额是 9.9 说的「真峰块换成损失位多的块」。对齐检查：b06 在 Ada 上 5.01e-06（H100 上 5.48e-06），b17 与 l17 9.06e-06，l4 1.54e-05，同配置三个 run 逐位相同。

墙钟（发射记录的 launch 时刻、`start` 与 `done` 事件的时刻、`wall_s`、末条 step 的 `train_s` 与 `ips`）：第一波 9 个 run 00:27 到 00:31 发射，`start` 事件 b17 00:41 到 00:43、l4 00:41 到 00:42、b06 00:45 到 00:46，也就是发射到 `start`（装模型、对齐检查、train 与 val 全集分词）13 到 15 分钟；l17 三个 01:32 到 01:33 发射、01:45 到 01:46 `start`。b17 `wall_s` 2789 到 2830 秒，`train_s` 1205 到 1237，`ips` 149 到 153，done 01:27 到 01:30；b06 `wall_s` 3618 到 3679，`train_s` 2332 到 2351，`ips` 78.3 到 78.9，done 01:45 到 01:46；l17 `wall_s` 2884 到 3078，`train_s` 1319 到 1360，`ips` 135 到 140，done 02:33 到 02:37；l4 `wall_s` 7081 到 7285，`train_s` 3885 到 3992，`ips` 46.1 到 47.4，done 02:39 到 02:43。整批从第一个发射到最后一个 done 是 2 小时 16 分钟。对 9.4 的估计：b06 在 Ada 开检查点估训练 71 到 86 分钟、整个 run 2.1 到 2.4 小时，实测训练 39 分钟、整个 run 61 分钟；b17 估训练 25 到 34 分钟、run 66 到 75 分钟，实测 20 到 21 分钟、47 分钟；l17 估训练 20 到 27 分钟、run 60 到 70 分钟，实测 22 到 23 分钟、48 到 51 分钟；l4 估训练 60 到 85 分钟、run 2 到 2.7 小时，实测 65 到 67 分钟、118 到 121 分钟；整批估 2 到 2.7 小时，实测 2 小时 16 分钟。`wall_s` 减 `train_s`（四次评估、epoch 末生成、存最好版本、探针）：b06 约 1,320 秒，b17 约 1,580，l17 约 1,560 到 1,720，l4 约 3,090 到 3,330。epoch 末 200 行生成的 `gen_s`：b06 55 到 67 秒，b17 270 到 292，l17 257 到 306，l4 216 到 342。

曲线（`eval` 事件的 `val_ce` 四个点、epoch 末 `val_exact_call`；`step` 事件的 `loss` 与 `grad_norm`）：12 个 run 的 `val_ce` 序列没有一处比前一点上升，12 个最好点都在第 4 个评估点。b06 lr1e-5 0.2358 → 0.2080 → 0.1844 → 0.1730（0.63），lr5e-5 0.2988 → 0.2316 → 0.2122 → 0.1857（0.615），lr2e-4 0.4921 → 0.3555 → 0.2999 → 0.2470（0.49）；b17 lr1e-5 0.2161 → 0.1880 → 0.1737 → 0.1636（0.65），lr5e-5 0.2815 → 0.2198 → 0.1972 → 0.1750（0.595），lr2e-4 0.4505 → 0.3229 → 0.2935 → 0.2269（0.54）；l17 lr1e-4 0.2384 → 0.2145 → 0.1928 → 0.1826（0.585），lr5e-4 0.2325 → 0.1963 → 0.1814 → 0.1647（0.63），lr2e-3 0.4343 → 0.4036 → 0.2770 → 0.2138（0.54）；l4 lr1e-4 0.2198 → 0.1979 → 0.1769 → 0.1671（0.63），lr5e-4 0.2137 → 0.1944 → 0.1750 → 0.1547（0.655），lr2e-3 2.6527 → 0.9333 → 0.7557 → 0.6510（0.335）。`loss` 与 `grad_norm` 没有非有限值。`grad_norm` 超过该 run 中位 5 倍的条数（`sweep_watch.py` 的规则）：六个全参 run 各 1 条，都是第 10 次更新那一条（b17 lr2e-4 是第 30 次），b06 三个 46.380 / 29.190 / 25.558 对中位 7.446 / 5.311 / 4.541，b17 三个 51.206 / 23.528 / 26.707 对 6.227 / 4.169 / 3.251；l17 lr2e-3 1 条，32.299 在第 220 次（中位 1.833），`loss` 首末 1.3272 → 0.2259、最小 0.2105；l4 lr2e-3 9 条，最大 77.698 在第 110 次（中位 1.749），`loss` 第 10 次 1.2364、第 30 次 0.4978（整程最小）、第 60 次 2.3015、第 70 次 4.2674（整程最大）、第 150 次 1.8884、第 250 次 1.0461、末条 0.5433；没有标记的 4 个 run（l17 lr1e-4 与 lr5e-4、l4 lr1e-4 与 lr5e-4）0 条。

显存与时间模型下一次要改的地方（按上面的实测）：第一，`tokens` 探针的数换算整程峰值要加损失位差额，不开检查点是 1.8 MB × (真峰块损失位 − 1,984)，开检查点要直接换成损失位多的块，9.8 与 9.9 已经这么算，`smoke_check.py` 的预期值也已经按块算，但 `stage-commands.md` 的排卡读法里还是 `worst_gb × 1.1` 一条；第二，`cost` 探针对整程峰值的低估在四个配置上是 0.001 到 0.42 GB（0.001% 到 1.9%），决定 25 的 1.1 倍是按 `tokens` 探针 7.5% 的偏差定的，`cost` 探针可以降到 1.05；第三，9.3 的时间倍率有三处和实测反向：Ada 开检查点对 H100 不开的倍率按 3.3 到 4 再乘 1.3 估，实测 2,332 秒对 0.6B H100 不开的 997 秒是 2.3 倍；1.7B 不开检查点估 1.5 到 2 倍，实测 1.21 到 1.24 倍；LoRA 估比全参快（0.8 到 1 倍），实测 l17 比 b17 慢 7% 到 10%（未合并的适配器每层多七对小矩阵乘）；4B LoRA 开检查点估 3.5 到 5 倍，实测 3.9 到 4.0 倍；第四，生成评估的时长随底座涨得比 9.6 估的多，全长提示下 1.7B 每批 8 条 10.3 到 12.3 秒、4B 8.7 到 13.7 秒，0.6B 2.2 到 2.7 秒，9.6 按评测脚本「底座大小几乎不影响」的判断在训练器里不成立（训练器是 fp32 权重逐步转 bf16、LoRA 未合并、提示左截到 8,096），1.7B 与 4B 一次生成评估要按 4.5 到 5.5 分钟算；第五，发射到 `start` 实测 13 到 15 分钟，四个底座相同，9.4 估的 17 / 20 / 25 分钟偏高，装模型与对齐检查在 4B 上没有明显更慢；第六，1.7B 开检查点的系数与固定项、4B 不开检查点的系数仍然没有实测，`smoke_check.py` 里那几个数还是推算值。

补扫（决定 40，按 9.7 的规则往网格下端加一档 2e-6，两个全参配置各一个 run；数从两个目录的 `train_log.jsonl` 与 `ops/runs.jsonl` 读）：`ks828b06_gptoss_cgen_lr2e-6`（Ada，开检查点）02:49 发射、03:04 `start`、04:02 done，`wall_s` 3494.7，`train_s` 2313.8，`ips` 79.6，`val_ce` 0.2859 → 0.2367 → 0.2237 → 0.2157，`val_exact_call` 0.545，`gen_s` 57.9，探针 22.005、峰值 22.427；`ks828b17_gptoss_cgen_lr2e-6`（H200，不开）02:50 发射、03:04 `start`、03:42 done，`wall_s` 2299.3，`train_s` 1128.8，`ips` 163.1，`val_ce` 0.2644 → 0.2249 → 0.2149 → 0.2046，`val_exact_call` 0.55，`gen_s` 263.0，探针 102.29、峰值 102.336。两个 run 的 `val_ce` 四个点都没有上升，探针与峰值和同配置的三个 run 逐位相同。全参两个配置四档的 epoch 末 `val_ce`（2e-6 / 1e-5 / 5e-5 / 2e-4）：b06 0.2157 / 0.1730 / 0.1857 / 0.2470，b17 0.2046 / 0.1636 / 0.1750 / 0.2269，最低都在 1e-5，两侧都高。
