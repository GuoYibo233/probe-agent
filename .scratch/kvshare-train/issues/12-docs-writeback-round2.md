...
3. `extending.md` §5 加 #28 到 #32，措辞照 spec 16.10 但按合并后的代码核对每条的触发条件；§3「换实现先例」段补一句：第二轮的四个开关都是参数、默认值等于推荐值、不新增格。
...
- `grep -n "overlong" .claude/skills/probe-pipeline/references/stage-commands.md MAP.md run.py` 三处命中；`grep -n "#32" .claude/skills/probe-pipeline/references/extending.md` 命中；`grep -n "worst_gb" .claude/skills/probe-pipeline/references/stage-commands.md` 命中。
