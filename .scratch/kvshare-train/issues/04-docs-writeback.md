...
3. `references/invariants.md`：先 `grep -n "4096\|accum 8\|32 个事件\|截断\|epochs 3\|bs 4"` 把所有写着旧口径的行找全（至少有：causal 超参那行的「bs 4 事件 / accum 8 / epochs 3」、cgen 目标构造那行的「按 max_length = 4096 − …左截」、上限那行），逐行改成新口径——上限（Comments 里的定值）、超长事件整条丢弃、一次更新 8 个事件（cgen/cparam 是 4 个事件一个逻辑小批 × 累积 2，ctool 是 bs 4 × accum 2）、cgen/cparam 1 个 epoch 每四分之一评一次、ctool 3 个 epoch、`--tok-budget` 定值。每处保留一句「np821 批用的是 4096 / 32 个事件 / 3 个 epoch」。
