...
2. `run.py` `TASKS` 加 `"sweep-lr"`：`desc="学习率扫描的清单(plan)与收表(report),发射仍走 gpu-run"`，`stage="train"`，`py="cprobe"`，`script` 指向新文件，`gpu=False`，`notes` 写两个子命令的用法与 `GRID` 常量在哪。字段照 `TASKS` 里其他 CPU 任务的条目逐项给全（`cmd_list / cmd_show` 直接下标取，缺一个 KeyError）。
3. 测试 `tests/test_sweep_lr.py`（纯 CPU，不需要 torch）：(a) `plan` 用默认 `GRID` 出 12 条，run_id 两两不同，每条 `cmd` 含 `--lr <该值>`，`lora` 为真的含 `--lora`、为假的不含，`--out` 以 run_id 结尾；`lr` 格式化 `1e-05 → 1e-5`、`2e-04 → 2e-4`、`5e-05 → 5e-5`；(b) `report` 在 `tempfile` 里手造两个 run 目录（一个有 `start` + 4 条 `eval` + `done` + 两条 `step` + `mem_probe_summary`，一个只有 `start` + 1 条 `eval`），出表：JSON 两条、`status` 各对、`*` 标在 `best_val_ce` 最低那行、缺 `done` 的行 `status == "running"`。
