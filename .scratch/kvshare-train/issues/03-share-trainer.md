...
4. 对齐检查按 spec 第 9 节的新写法：抽中事件的原始行按 `(event, sent_idx)` 升序写临时 jsonl，同一份文件喂 `share_data.load_events(limit=0)` 和旧脚本的 `CallDS` / `ParamDS(limit=0)`，按位置配对并逐位断言 `text` 相同、丢弃计数相同，再比逐行 ce；fp32 下关 TF32；`ALIGN_CHECK.json` 的键用大写 `PASS`。
...
- `cprobe-env/bin/python pipeline/train/train_causal_share.py --mode cgen --data pipeline/data/nyapass_aw_v1/gptoss --out pipeline/runs/smoke/share_cpu_cgen_smoke --device cpu --smoke --max-events 6 --log-every 1 --align-events 2 --base qwen --force` 在 CPU 上跑通（产物目录是 NFS 软链下的既有冒烟落点，0.6B fp32 的 `best/` 约 2.4 GB 不许写 /tmp 或 home；0.6B fp32 在 CPU 上 6 个短事件可以接受几分钟；`--smoke` 按全文 token 数升序取前 N 个，所以 6 个事件都是最短的），产出 `ALIGN_CHECK.json`（`PASS` 真，`max_abs_diff ≤ 1e-5`，`n_rows` 等于抽中事件在参照路径存活的行数）、`train_log.jsonl` 五种事件、`best/meta.json`（含 `trainer: "share"`、`call_sep`、`max_len`、`data`）。cparam 同样跑一次，`--out pipeline/runs/smoke/share_cpu_cparam_smoke`。
